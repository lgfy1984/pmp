 ...
 * 
 */
public class ProjectFinanceCostServiceImpl implements IProjectFinanceCostService{
	...
	private IProjectFinanceCostDAO projectFinanceCostDAO;

	
	public IProjectFinanceCostDAO getProjectFinanceCostDAO() {
		return projectFinanceCostDAO;
	}

	public void setProjectFinanceCostDAO(
			IProjectFinanceCostDAO projectFinanceCostDAO) {
		this.projectFinanceCostDAO = projectFinanceCostDAO;
	}
	...
	* @author lengj
	...
	public void initForm(ProjectFinanceCostForm form){
		...
		List<?> list1 = projectFinanceCostDAO.getTaskClassDict(form.getProjectClassCode());
	...
	* @author lengj
	...
	public void save(ProjectFinanceCostForm form){
		ProjectFinanceCost data = projectFinanceCostDAO.findById(form.getIdHidden());
		if(data!=null){
			data.setSeqNo(Converter.toInteger(form.getSeqNo()));
			this.setData(form, data);
			projectFinanceCostDAO.update(data);
		}else{
		
			data = new ProjectFinanceCost();
			...
			setData(form, data);
			projectFinanceCostDAO.save(data);
	...
	* @author lengj
	...
	public void saveExcelData(ProjectFinanceCostForm form){
		for (ProjectFinanceVo pfv : form.getPfv()) {
			ProjectFinanceCost data = new ProjectFinanceCost();
	...
	* @author lengj
	 */
	public void saveKnowledge(ProjectFinanceCostForm form){
	...
	* @author lengj
	...
	public ProjectFinanceCost findDetailById(String id){
		...
		return projectFinanceCostDAO.findById(id);
	}
	/**
	 * 删除
	*
	...
	public void delete(ProjectFinanceCostForm form){
		...
		if(objs!=null && objs.length>0){
			for(int i=0;i<objs.length;i++){
				ProjectFinanceCost data = projectFinanceCostDAO.findById(objs[i]);
				projectFinanceCostDAO.delete(data);
			}
		}
	...
	* @author lengj
	...
	public int getProjectFinanceCostCount(String projectNameCase,String projectBaseinfoId, String projectClassCode,
		...
		return projectFinanceCostDAO.getProjectFinanceCostCount(projectNameCase,projectBaseinfoId, projectClassCode, staffName, startTime, endTime, userId,timeCase,timeSelect);
	}
	/**
	 * 修改初始化
	*
	...
	* @author lengj
	 ...
	 * 咨询信息列表
	*
	* @Title: getProjectFinanceCostSearch
	* @param hosform
	* @param curCount
	* @param pageSize
	* @return void
	...
	* @author lengj
	...
	public void getProjectFinanceCostSearch(ProjectFinanceCostForm hosform, int curCount,int pageSize){
			...
			initForm(hosform);
			List<?> list = projectFinanceCostDAO
					.getProjectFinanceCostData(hosform.getProjectNameCase(), hosform.getProjectBaseinfoIdCase(),  hosform.getProjectClassCodeCase(),
		    ...
		    if (list != null && list.size() > 0) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					temp.setTaskCode(Converter.toBlank(obs[7]));
					temp.setTaskName(this.changeClassId(Converter.toBlank(obs[7]), hosform));
					temp.setSeqNo(Converter.toBlank(obs[6]));
					temp.setCosts(Converter.toBlank(obs[9]));
					if(obs[8]!=null && !obs[8].equals("")){
						temp.setWorkDate(sdf.format(obs[8]));
					}
					temp.setStaffCode(Converter.toBlank(obs[11]));
					temp.setWorkStaffName(Converter.toBlank(obs[10]));
					temp.setCreateUserId(Converter.toBlank(obs[3]));
					temp.setCreateUserName(Converter.toBlank(Converter.toBlank(obs[4])));
					crd.add(temp);
				}
				hosform.setPfv(crd);
			}
	...
	/** 构造exceldata */
	private void setData(ProjectFinanceVo pfv,ProjectFinanceCost data,ProjectFinanceCostForm form) {
...
//		data.setChargeType(Converter.toBlank(obs[1]));
	...
	/** 构造data */
	private void setData(ProjectFinanceCostForm form,
			ProjectFinanceCost data) {
	...
	/** 构造data */
	private void setForm(ProjectFinanceCostForm form,
			ProjectFinanceCost data) {
	...
	private String changeClassId(String id, ProjectFinanceCostForm form) {
