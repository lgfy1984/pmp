...
import com.tianjian.comm.dao.ICommDictPublicDoubleDAO;
import com.tianjian.comm.struts.form.CommDictPublicDoubleForm;

public class CommDictPublicDoubleServiceImpl implements ICommDictPublicDoubleService {
	private static final Logger log = LogManager.getLogger(CommDictPublicDoubleServiceImpl.class);
	private ICommDictPublicDoubleDAO commDictPublicDoubleDAO;
	private ICommDictPublicClassDAO commDictPublicClassDAO;

	//------------------------DAO-- 声明结束---------------------------------------------------
	

	public ICommDictPublicDoubleDAO getCommDictPublicDoubleDAO() {
		return commDictPublicDoubleDAO;
	}
	
	public void setCommDictPublicDoubleDAO(ICommDictPublicDoubleDAO commDictPublicDoubleDAO) {
		this.commDictPublicDoubleDAO = commDictPublicDoubleDAO;
	}
   ...
   /**新增之前的初始化，这里需要初始化字典库内容*/
	public void addInit(CommDictPublicDoubleForm form){
		init(form);
		form.setSeqNo(String.valueOf(this.commDictPublicDoubleDAO.getMaxSeqNo()));
	}
	...
	public CommDictPublicDouble checkData(String id){
		CommDictPublicDouble data = commDictPublicDoubleDAO.findById(id);
		return data;
	}
	
	/**提交保存*/
	public void save(CommDictPublicDoubleForm form){
		CommDictPublicDouble data = new CommDictPublicDouble();
		this.setData(form, data);
		commDictPublicDoubleDAO.save(data);
	}
	/**修改之前初始化*/
	public void updateInit(CommDictPublicDoubleForm form){
		CommDictPublicDouble data = commDictPublicDoubleDAO.findById(form.getIdHidden());
	...
	public void update(CommDictPublicDoubleForm form){
		CommDictPublicDouble data = commDictPublicDoubleDAO.findById(form.getIdHidden());
		this.setData(form, data);
		commDictPublicDoubleDAO.update(data);
	}
	/**提交察看详细*/
	...
	public void delete(CommDictPublicDoubleForm form){
		CommDictPublicDouble data = commDictPublicDoubleDAO.findById(form.getIdHidden());
		commDictPublicDoubleDAO.delete(data);
	}
	/**获取总记录数*/
	public int getCount(String id, String classCode,String dictCode, String dictValue, String inputCode, String seqNo){
		return commDictPublicDoubleDAO.getCount(id,classCode, dictCode, dictValue, inputCode, seqNo);
	}
	/**根据查询条件获取当前显示页的数据*/
	public void getSearch(CommDictPublicDoubleForm form, int curCount, int pageSize){
		...
		List<?> list = commDictPublicDoubleDAO.getData(form.getId()
								, form.getClassCode()
								, form.getDictCode()
								, form.getDictValue()
								, form.getInputCode()
	...
	/**构造data*/
	private void setData(CommDictPublicDoubleForm form, CommDictPublicDouble data){
		try{
			...
			data.setDictValue  (Double.valueOf((form.getDictValue() == null || "".equals(form.getDictValue().trim())) ? "0" : form.getDictValue()));
	...
	/**在传递数据到页面之前构造form*/
	private void setForm(CommDictPublicDoubleForm form, CommDictPublicDouble data){
		try{
	...
	/**在初始化阶段将字典库传入form中*/
	private void init(CommDictPublicDoubleForm form){
		...
		if (form.getSeqNo() == null || form.getSeqNo().equals(""))
			form.setSeqNo(commDictPublicDoubleDAO.getMaxSeqNo() + "");

	}

	/**获取查询结果之前初始化*/
	private void searchInit(CommDictPublicDoubleForm form){
		this.getCommDictPulicClass();
		form.setClassCodes(this.getClassCodes());
		form.setClassNames(this.getClassNames());
