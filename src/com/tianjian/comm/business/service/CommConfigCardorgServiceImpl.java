...
import com.tianjian.comm.business.ICommConfigCardorgService;
import com.tianjian.comm.dao.ICommConfigCardorgDAO;
import com.tianjian.comm.struts.form.CommConfigCardorgForm;

public class CommConfigCardorgServiceImpl implements ICommConfigCardorgService {
	private static final Logger log = LogManager.getLogger(CommConfigCardorgServiceImpl.class);
	private ICommConfigCardorgDAO commConfigCardorgDAO;

	//------------------------DAO-- 声明结束---------------------------------------------------
	
	
	public ICommConfigCardorgDAO getCommConfigCardorgDAO() {
		return commConfigCardorgDAO;
	}

	
	public void setCommConfigCardorgDAO(ICommConfigCardorgDAO commConfigCardorgDAO) {
		this.commConfigCardorgDAO = commConfigCardorgDAO;
	}
	...
	public CommConfigCardorg checkData(String id){
		CommConfigCardorg data = commConfigCardorgDAO.findById(id);
		return data;
	}
	/**提交保存*/
	public void save(CommConfigCardorgForm form){
		CommConfigCardorg data = new CommConfigCardorg();
		this.setData(form, data);
		commConfigCardorgDAO.save(data);
	}
	/**修改之前初始化*/
	public void updateInit(CommConfigCardorgForm form){
		CommConfigCardorg data = commConfigCardorgDAO.findById(form.getItemCodeHidden());
	...
	public void update(CommConfigCardorgForm form){
		CommConfigCardorg data = commConfigCardorgDAO.findById(form.getItemCodeHidden());
		this.setData(form, data);
		commConfigCardorgDAO.update(data);
	}
	/**提交察看详细*/
	...
	public void delete(CommConfigCardorgForm form){
		CommConfigCardorg data = commConfigCardorgDAO.findById(form.getItemCodeHidden());
		commConfigCardorgDAO.delete(data);
	}
	/**获取总记录数*/
	public int getCount(String itemCode, String itemName, String inputCode, String seqNo){
		return commConfigCardorgDAO.getCount(itemCode, itemName, inputCode, seqNo);
	}
	/**根据查询条件获取当前显示页的数据*/
	public void getSearch(CommConfigCardorgForm form, int curCount, int pageSize){
		...
		List<?> list = commConfigCardorgDAO.getData(form.getItemCode()
								, form.getItemName()
								, form.getInputCode()
								, form.getSeqNo()
								, order
	...
	/**构造data*/
	private void setData(CommConfigCardorgForm form, CommConfigCardorg data){
		try{
	...
	/**在传递数据到页面之前构造form*/
	private void setForm(CommConfigCardorgForm form, CommConfigCardorg data){
		try{
	...
	/**在初始化阶段将字典库传入form中*/
	private void init(CommConfigCardorgForm form){
		if (form.getSeqNo() == null || form.getSeqNo().equals(""))
			form.setSeqNo(commConfigCardorgDAO.getMaxSeqNo() + "");
	}
	/***/
	...
	/**获取查询结果之前初始化*/
	/**在进行查找之前将字典库传入form中*/
	private void searchInit(CommConfigCardorgForm form){
	}
