...
public class CommConfigHospitalTypeServiceImpl implements
	...
	// private String COMMCONFIGHOSPITALTYPELEVEL_1 =
	// CommInit.getProperty("COMMCONFIGHOSPITALTYPELEVEL_1");
	// private String COMMCONFIGHOSPITALTYPELEVEL_2 =
	// CommInit.getProperty("COMMCONFIGHOSPITALTYPELEVEL_2");
	// private String COMMCONFIGHOSPITALTYPELEVEL_3 =
	// CommInit.getProperty("COMMCONFIGHOSPITALTYPELEVEL_3");
	// private String COMMCONFIGHOSPITALTYPELEVEL_ALL =
	// CommInit.getProperty("COMMCONFIGHOSPITALTYPELEVEL_ALL");
	/***/
	public void setData(CommConfigHospitalTypeForm form,
			CommConfigHospitalType data, HttpServletRequest request) {
		...
		if (form.getLevelCode_1() != null && form.getLevelCode_2() != null
				&& form.getLevelCode_1().trim().length() > 0
				&& form.getLevelCode_2().trim().length() > 0) {
			// data.setLevelFlag(Long.valueOf(COMMCONFIGHOSPITALTYPELEVEL_3));
			data.setLevelFlag(Long.valueOf((String) application
					.getAttribute("comm.COMMCONFIGHOSPITALTYPELEVEL_3")));
			data.setParentItemCode(form.getLevelCode_2());
		} else if (form.getLevelCode_1() != null
				&& form.getLevelCode_1().trim().length() > 0) {
			// data.setLevelFlag(Long.valueOf(COMMCONFIGHOSPITALTYPELEVEL_2));
			data.setLevelFlag(Long.valueOf((String) application
					.getAttribute("comm.COMMCONFIGHOSPITALTYPELEVEL_2")));
		...
		try {
			data
					.setSeqNo(Long.valueOf(this.transNullToString(form
							.getSeqNo())));
		} catch (Exception e) {
			data.setSeqNo(this.commConfigHospitalTypeDAO
					.seqNoMaker("CommConfigHospitalType"));
		}
	}

	/***/
	public void setForm(CommConfigHospitalTypeForm form,
			CommConfigHospitalType data, HttpServletRequest request) {
		ServletContext application = request.getSession().getServletContext();
		...
		form
				.setParentItemCode(this.transNullToString(data
						.getParentItemCode()));

		// if(data.getLevelFlag().equals(Long.valueOf(COMMCONFIGHOSPITALTYPELEVEL_1))){
		if (data.getLevelFlag().equals(
				Long.valueOf((String) application
						.getAttribute("comm.COMMCONFIGHOSPITALTYPELEVEL_1")))) {
			form.setLevelCode_1("");
			form.setLevelCode_2("");
			form.setParentItemName("");
			// }else
			// if(data.getLevelFlag().equals(Long.valueOf(COMMCONFIGHOSPITALTYPELEVEL_2))){
		} else if (data.getLevelFlag().equals(
				Long.valueOf((String) application
						.getAttribute("comm.COMMCONFIGHOSPITALTYPELEVEL_2")))) {
			form.setLevelCode_1(data.getParentItemCode());
			form.setLevelCode_2("");
			form.setParentItemName(this
					.transNullToString(this.commConfigHospitalTypeDAO
							.findP2ValueByP1("CommConfigHospitalType",
									"itemCode", data.getParentItemCode(),
									"itemName")));
			// }else
			// if(data.getLevelFlag().equals(Long.valueOf(COMMCONFIGHOSPITALTYPELEVEL_3))){
		} else if (data.getLevelFlag().equals(
				Long.valueOf((String) application
						.getAttribute("comm.COMMCONFIGHOSPITALTYPELEVEL_3")))) {
			form.setLevelCode_1(this.commConfigHospitalTypeDAO.findP2ValueByP1(
					"CommConfigHospitalType", "itemCode", data
							.getParentItemCode(), "parentItemCode"));
	...
	/** get 中类 */
	public List<CommConfigHospitalType> getLevel2(
			CommConfigHospitalTypeForm form, HttpServletRequest request) {
		List<CommConfigHospitalType> level2 = null;

		if (form.getLevelCode_1() != null
				&& form.getLevelCode_1().trim().length() > 0) {
			return this.commConfigHospitalTypeDAO.getByParentItemCode(form
					.getLevelCode_1().trim(), request);
		} else {
			return null;
		}
	...
	public void init(CommConfigHospitalTypeForm form, HttpServletRequest request) {
		List<?> level1 = this.commConfigHospitalTypeDAO.getByParentItemCode("",
				request);
		List<?> level2 = null;
		...
		if (level1 != null && level1.size() > 0) {
			...
			for (int i = 0; i < level1.size(); i++) {
				levelCode_1List[i + 1] = ((CommConfigHospitalType) level1
						.get(i)).getItemCode();
				levelName_1List[i + 1] = ((CommConfigHospitalType) level1
						.get(i)).getItemName();
			}
		} else {
		...
		if (form.getLevelCode_1() != null
				&& form.getLevelCode_1().trim().length() > 0) {
			level2 = this.commConfigHospitalTypeDAO.getByParentItemCode(form
					.getLevelCode_1().trim(), request);
			if (level2 != null && level2.size() > 0) {
				...
				for (int i = 0; i < level2.size(); i++) {
					levelCode_2List[i + 1] = ((CommConfigHospitalType) level2
							.get(i)).getItemCode();
					levelName_2List[i + 1] = ((CommConfigHospitalType) level2
							.get(i)).getItemName();
				}
			} else {
	...
	/** 添加初始化 */
	public void addInit(CommConfigHospitalTypeForm form,
			HttpServletRequest request) {
		this.init(form, request);
		form.setSeqNo(String.valueOf(this.commConfigHospitalTypeDAO
				.seqNoMaker("CommConfigHospitalType")));
	}

	/** 更新初始化 */
	public void updateInit(CommConfigHospitalTypeForm form,
			HttpServletRequest request) {
		this.init(form, request);
	}
	...
	/** 更新 */
	public void update(CommConfigHospitalTypeForm form,
			HttpServletRequest request) {
		CommConfigHospitalType data = this.findByItemCode(form.getItemCode()
				.trim());
		this.setData(form, data, request);
	...
	public void delete(CommConfigHospitalTypeForm form) {
		CommConfigHospitalType data = this.findByItemCode(form
				.getItemCodeHidden().trim());
	...
	public void getSearch(CommConfigHospitalTypeForm form, int curCount,
		...
		List<?> list = this.commConfigHospitalTypeDAO.getData(form
				.getItemCodeQuery(), form.getItemNameQuery(), form
				.getInputCodeQuery(), form.getSeqNoQuery(), order, curCount,
				pageSize);
		if (list != null && list.size() > 0) {
			...
			for (int i = 0; i < list.size(); i++) {
				...
				parentItemCodes[i] = String
						.valueOf(transNullToString(((Object[]) list.get(i))[4]));
				commentss[i] = String
						.valueOf(transNullToString(((Object[]) list.get(i))[5]));
				seqNos[i] = ((Object[]) list.get(i))[6] == null ? "0" : String
						.valueOf(((Object[]) list.get(i))[6]);
			}
	...
	public int getCount(CommConfigHospitalTypeForm form) {
		return this.commConfigHospitalTypeDAO.getCount(form.getItemCodeQuery(),
				form.getItemNameQuery(), form.getInputCodeQuery(), form
						.getSeqNoQuery());
	}

	/** 弹出层所用 */
	public String getXml(String flag, String input, String hspTypeLevel,
			HttpServletRequest request) {
		StringBuffer buffer = new StringBuffer();
		ServletContext application = request.getSession().getServletContext();
		// List<?> list = this.commConfigHospitalTypeDAO.findHspTypeList(flag,
		// input, hspTypeLevel,
		// Integer.valueOf(CommInit.getProperty("CURRECORD_TANCHUCENG")),
		// CommInit.getPageSize("PAGE_SIZE_TANCHUCENG"));
		List<?> list = this.commConfigHospitalTypeDAO.findHspTypeList(flag,
				input, hspTypeLevel, Integer.valueOf((String) application
						.getAttribute("comm.CURRECORD_TANCHUCENG")), Integer
						.valueOf((String) application
								.getAttribute("comm.PAGE_SIZE_TANCHUCENG")),
				request);
		buffer.append("<response>");
		for (int i = 0; i < list.size(); i++) {
			CommConfigHospitalType charbean = (CommConfigHospitalType) list
					.get(i);
			// -----以下xml标签不要改变--------------------------------------------------------------
			buffer.append("<ress>");
			buffer.append("<resInputCode>" + charbean.getInputCode()
					+ "</resInputCode>");
			buffer.append("<resItemCode>" + charbean.getItemCode()
					+ "</resItemCode>");
			buffer.append("<resItemName>" + charbean.getItemName()
					+ "</resItemName>");
			buffer.append("<resItemId>" + charbean.getItemCode()
					+ "</resItemId>");
