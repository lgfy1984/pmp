...
import com.tianjian.comm.dao.ICommConfigVipDAO;
import com.tianjian.comm.struts.form.CommConfigSexForm;

public class CommConfigVipServiceImpl implements ICommConfigVipService {
	private static final Logger log = LogManager.getLogger(CommConfigSexServiceImpl.class);
	private ICommConfigVipDAO commConfigVipDAO;
	...
	public ICommConfigVipDAO getCommConfigVipDAO() {
		return commConfigVipDAO;
	}

	public void setCommConfigVipDAO(ICommConfigVipDAO commConfigVipDAO) {
		this.commConfigVipDAO = commConfigVipDAO;
	}
	...
	/**获取记录对象*/
	public CommConfigVip checkData(String id){
		CommConfigVip data = commConfigVipDAO.findById(id);
		return data;
	}
	/**提交保存*/
	public void save(CommConfigSexForm form){
		CommConfigVip data = new CommConfigVip();
		this.setData(form, data);
		commConfigVipDAO.save(data);
	}
	/**修改之前初始化*/
	public void updateInit(CommConfigSexForm form){
		CommConfigVip data = commConfigVipDAO.findById(form.getItemCodeHidden());
	...
	public void delete(CommConfigSexForm form){
		CommConfigVip data = commConfigVipDAO.findById(form.getItemCodeHidden());
		commConfigVipDAO.delete(data);
	}
	/**获取总记录数*/
	public int getCount(String itemCode, String itemName, String inputCode, String seqNo){
		return commConfigVipDAO.getCount(itemCode, itemName, inputCode, seqNo);
	}
	/**根据查询条件获取当前显示页的数据*/
	public void getSearch(CommConfigSexForm form, int curCount, int pageSize){
		...
		List<?> list = commConfigVipDAO.getData(form.getItemCode()
								, form.getItemName()
								, form.getInputCode()
								, form.getSeqNo()
								, order
	...
	/**构造data*/
	private void setData(CommConfigSexForm form, CommConfigVip data){
		try{
	...
	/**在传递数据到页面之前构造form*/
	private void setForm(CommConfigSexForm form, CommConfigVip data){
		try{
	...
	private void init(CommConfigSexForm form){
		if (form.getSeqNo() == null || form.getSeqNo().equals(""))
			form.setSeqNo(commConfigVipDAO.getMaxSeqNo() + "");
