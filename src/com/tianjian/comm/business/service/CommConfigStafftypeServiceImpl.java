...
public class CommConfigStafftypeServiceImpl implements
		ICommConfigStafftypeService {
	private static final Logger log = LogManager
			.getLogger(CommConfigStafftypeServiceImpl.class);
	private ICommConfigStafftypeDAO commConfigStafftypeDAO;

	// ------------------------DAO--
	// 声明结束---------------------------------------------------

	public ICommConfigStafftypeDAO getCommConfigStafftypeDAO() {
		return commConfigStafftypeDAO;
	}

	public void setCommConfigStafftypeDAO(
			ICommConfigStafftypeDAO commConfigStafftypeDAO) {
		this.commConfigStafftypeDAO = commConfigStafftypeDAO;
	}

	public void addInit(CommConfigStafftypeForm form) {
		init(form);
		form.setSeqNo(String.valueOf(this.commConfigStafftypeDAO.seqNoMaker("CommConfigStafftype")));
	...
	public CommConfigStafftype checkData(String id) {
		CommConfigStafftype data = commConfigStafftypeDAO.findById(id);
		return data;
	}

	/** 获取记录对象 */
	public CommConfigStafftype checkDataByName(String itemName) {
		CommConfigStafftype data = commConfigStafftypeDAO
	...
	public void save(CommConfigStafftypeForm form) {
		CommConfigStafftype data = new CommConfigStafftype();
		this.setData(form, data);
		commConfigStafftypeDAO.save(data);
	}

	/** 修改之前初始化 */
	public void updateInit(CommConfigStafftypeForm form) {
		CommConfigStafftype data = commConfigStafftypeDAO.findById(form
				.getItemCodeHidden());
	...
	public void update(CommConfigStafftypeForm form) {
		CommConfigStafftype data = commConfigStafftypeDAO.findById(form
				.getItemCodeHidden());
		this.setData(form, data);
		commConfigStafftypeDAO.update(data);
	}
	...
	public void delete(CommConfigStafftypeForm form) {
		CommConfigStafftype data = commConfigStafftypeDAO.findById(form
				.getItemCodeHidden());
		commConfigStafftypeDAO.delete(data);
	}

	/** 获取总记录数 */
	public int getCount(String itemCode, String itemName, String inputCode,
			String seqNo) {
		return commConfigStafftypeDAO.getCount(itemCode, itemName, inputCode,
				seqNo);
	}

	/** 根据查询条件获取当前显示页的数据 */
	public void getSearch(CommConfigStafftypeForm form, int curCount,
			int pageSize) {
		try {
			...
			List<?> list = commConfigStafftypeDAO.getData(form.getItemCode(),
			...
			if (list != null && list.size() > 0) {
				...
				List<CommConfigStafftype> ccsList = new ArrayList<CommConfigStafftype>();
				for (int i = 0; i < list.size(); i++) {
					CommConfigStafftype ccs = new CommConfigStafftype();
					ccs
							.setItemCode(String
	...
	/** 构造data */
	private void setData(CommConfigStafftypeForm form, CommConfigStafftype data) {
		try {
	...
	/** 在传递数据到页面之前构造form */
	private void setForm(CommConfigStafftypeForm form, CommConfigStafftype data) {
		try {
	...
	/** 在初始化阶段将字典库传入form中 */
	private void init(CommConfigStafftypeForm form) {
		if (form.getSeqNo() == null || form.getSeqNo().equals(""))
			form.setSeqNo(commConfigStafftypeDAO.getMaxSeqNo() + "");
	...
	/** 获取查询结果之前初始化 */
	/** 在进行查找之前将字典库传入form中 */
	private void searchInit(CommConfigStafftypeForm form) {
	}
