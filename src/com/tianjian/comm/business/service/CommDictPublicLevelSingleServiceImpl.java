...
public class CommDictPublicLevelSingleServiceImpl implements ICommDictPublicLevelSingleService {
	private static final Logger log = LogManager.getLogger(CommDictPublicLevelSingleServiceImpl.class);
   ...
   /**新增之前的初始化，这里需要初始化字典库内容*/
	...
	private void init(CommDictPublicLevelForm form){
		 form.setTableName(transNullToString(commDictPublicClassDAO.getClassName(form.getTableCode())));
		this.getCommDictPulicLevel(form.getTableCode());
	...
	/**根据字典类别获取字典详细信息列表*/
	public void getCommDictPulicLevel(String tableCode){
		List<?> data = commDictPublicLevelDAO.findAllByTableCode(tableCode);
		String[] tempId = null;
