...
public class CommConfigHospitalTypeAction extends BaseAction {
	...
	private ActionForward add(ActionMapping mapping, ActionForm form,
		...
		try {
			CommConfigHospitalTypeForm cchtform = (CommConfigHospitalTypeForm) form;
			CommConfigHospitalType data = this.commConfigHospitalTypeService
					.findByItemCode(cchtform.getItemCode().trim());
			if (data != null) {
				String message = ResourcesUtil.getValue("conf.comm.CommLocale",
						"comm.java.CommConfigHospitalTypeAction.codeExist",
						request)
						+ "!";
				cchtform.setMessage(message);
				return this.addInit(mapping, form, request, response);
			} else {
				cchtform.setInputCode(commConfigInputDictService
						.getInputCode(cchtform.getItemName()));
				this.commConfigHospitalTypeService.save(cchtform, request);
				cchtform = new CommConfigHospitalTypeForm();
				String message = ResourcesUtil.getValue("conf.comm.CommLocale",
						"comm.java.commom.addSuccess", request)
						+ "!";
				cchtform.setMessage(message);
				request.setAttribute("data", cchtform);
				return this.query(mapping, form, request, response);
	...
	private ActionForward delete(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			cchtform = new CommConfigHospitalTypeForm();
			String message = ResourcesUtil.getValue("conf.comm.CommLocale",
					"comm.java.commom.deleteSuccess", request)
					+ "!";
			cchtform.setMessage(message);
			return this.query(mapping, form, request, response);
		} catch (Exception e) {
	...
	private ActionForward detail(ActionMapping mapping, ActionForm form,
		...
		try {
			CommConfigHospitalTypeForm cchtform = (CommConfigHospitalTypeForm) form;
			CommConfigHospitalType data = this.commConfigHospitalTypeService
					.findByItemCode(cchtform.getItemCodeHidden());
	...
	private ActionForward update(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			cchtform = new CommConfigHospitalTypeForm();
			String message = ResourcesUtil.getValue("conf.comm.CommLocale",
					"comm.java.CommConfigHospitalTypeAction.changeSucceess",
					request)
					+ "!";
			cchtform.setMessage(message);
			request.setAttribute("data", cchtform);
...
//			}
			int recordCount = this.commConfigHospitalTypeService
					.getCount(cchtform);
			...
			if (request.getSession().getAttribute(
				...
				pageSize = Integer.parseInt((String) request.getSession()
						.getAttribute("page_282881f53463c89c013463c89cbe0000"));
			} else {
				ServletContext application = request.getSession()
						.getServletContext();
				pageSize = Integer.parseInt((String) application
						.getAttribute("comm.PAGE_SIZE"));
			}
			...
			this.commConfigHospitalTypeService.getSearch(cchtform, count,
					pageSize);
			request.setAttribute("data", cchtform);
			return mapping.findForward("query");
		} catch (Exception e) {
	...
	private ActionForward setLevel2(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			List<CommConfigHospitalType> level2List = this.commConfigHospitalTypeService
					.getLevel2(cchtform, request);

			JSONArray jsonArray = null;
			response.setCharacterEncoding("utf-8");
			jsonArray = JSONArray.fromObject(level2List);
			System.out.println(jsonArray.toString());
			response.getWriter().print(jsonArray);// JSON返回数据
			response.flushBuffer();
			// String xmlString = getXMLData(cchtform.getLevelCode_2List(),
			// cchtform.getLevelName_2List());
	...
	private ActionForward getHspType(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			String xmlString = this.commConfigHospitalTypeService.getXml(flag,
					input, hspTypeLevel, request);
	...
	public void writeResponse(HttpServletResponse response, String xmlString)
			throws IOException {
