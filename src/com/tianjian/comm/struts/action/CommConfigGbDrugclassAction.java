...
import com.tianjian.comm.business.ICommConfigGbDrugclassService;
 ...
 * @author DZENALL
 */
public class CommConfigGbDrugclassAction extends Action{
	
	private ICommConfigGbDrugclassService commConfigGbDrugclassService;
	private ICommConfigInputDictService commConfigInputDictService;
	public ICommConfigGbDrugclassService getCommConfigGbDrugclassService(){
		return commConfigGbDrugclassService;
	}
	public void setCommConfigGbDrugclassService(ICommConfigGbDrugclassService commConfigGbDrugclassService){
		this.commConfigGbDrugclassService = commConfigGbDrugclassService;
	}
...
//		System.out.println("-----------verbId==" + verbId);
	...
	private ActionForward updateInit(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response){
		try{
			...
			commConfigGbDrugclassService.updateInit(ccdform);
	...
	public ActionForward update(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response){
		try{
			...
			commConfigGbDrugclassService.update(ccdform);
			ccdform.setItemCode("");
			ccdform.setItemName("");
			ccdform.setComments("");
	...
	public ActionForward delete(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response){
		try{
			CommConfigGbDrugclassForm ccdform = (CommConfigGbDrugclassForm)form;
			commConfigGbDrugclassService.delete(ccdform);
			ccdform.setItemCode("");
			return  this.query(mapping, ccdform, request, response);
		}catch(Exception e) {
			return mapping.findForward("fail");
	...
	public ActionForward addInit(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response){
		...
		commConfigGbDrugclassService.getData(ccdform2);
	...
	public ActionForward add(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) {
		try{
			CommConfigGbDrugclassForm ccdform = (CommConfigGbDrugclassForm)form;
			CommConfigGbDrugclass  checkData = commConfigGbDrugclassService.checkData(ccdform);
			if(checkData != null){
			...
			ccdform.setInputCode(this.getCommConfigInputDictService().getInputCode(ccdform.getItemName()));
			commConfigGbDrugclassService.save(ccdform);
			ccdform.setItemCode("");
			ccdform.setItemName("");
			ccdform.setSeqNo("");
			ccdform.setInputCode("");
...
////////		page start   ////////////////////////
			...
			int count;
			int page = 0;
			int recordCount = commConfigGbDrugclassService.getCount(ccdform.getItemCode(), ccdform.getItemName(), ccdform.getInputCode());
...
////////		page end   ////////////////////////
			...
			commConfigGbDrugclassService.getSearch(ccdform, count, pageSize);
			request.setAttribute("data", ccdform);	
