...
public class CommConfigCountryDAO extends HibernateDaoSupport implements  ICommConfigCountryDAO{

	private static final Logger log = LogManager.getLogger(CommConfigCountryDAO.class);
	
	public CommConfigCountry findById(String id){
		try{
			CommConfigCountry fac = null;
			List<?> ls = getHibernateTemplate().find(" from CommConfigCountry a where a.itemCode = ? ", id);
	...
	public String getItemName(String id){
		String itemName = "";
		try {
			List<?> l = getHibernateTemplate().find(" from CommConfigCountry aa where aa.itemCode = ? ", id);
			if(l != null && l.size() > 0){
				CommConfigCountry pub = (CommConfigCountry)l.get(0);
    ...
    public List<?> findAll(){
    	try {
    		String sql = " from CommConfigCountry a  ";
    ...
    public List<?> getData(String itemCode, String itemName, String inputCode, String seqNo, String orderNo, int curCount,int pageSize){
    	try {
	    	...
	    	sql .append( " from CommConfigCountry a");
    ...
    public int getMaxSeqNo(){
		int temp = 1;
		try {
			List<?> l = getHibernateTemplate().find(" select max(aa.seqNo) from CommConfigCountry aa ");
			if(l != null && l.size() > 0){
				Object o = l.get(0);
				if(o==null){
					return temp;
