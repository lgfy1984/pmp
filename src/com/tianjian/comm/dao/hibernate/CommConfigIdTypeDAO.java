...
public class CommConfigIdTypeDAO extends HibernateDaoSupport implements  ICommConfigIdTypeDAO{

	private static final Logger log = LogManager.getLogger(CommConfigIdTypeDAO.class);
	
	public CommConfigIdType findById(String id){
		try{
			CommConfigIdType fac = null;
			List<?> ls = getHibernateTemplate().find(" from CommConfigIdType a where a.itemCode = ? ", id);
			if(ls != null && ls.size() >0){
				fac = (CommConfigIdType) ls.get(0);
			}
	...
	public String getItemName(String id){
		String itemName = "";
		try {
			List<?> l = getHibernateTemplate().find(" from CommConfigIdType aa where aa.itemCode = ? ", id);
			if(l != null && l.size() > 0){
				CommConfigIdType pub = (CommConfigIdType)l.get(0);
    ...
    public void save(CommConfigIdType commConfigCountry){
    	try {
    ...
    public void update(CommConfigIdType commConfigCountry){
    	try {
    ...
    public void delete(CommConfigIdType commConfigCountry){
    	try {
    ...
    public List<?> findAll(){
    	try {
    		String sql = " from CommConfigIdType a  ";
    ...
    public List<?> getData(String itemCode, String itemName, String inputCode, String seqNo, String orderNo, int curCount,int pageSize){
    	try {
	    	...
	    	sql .append( " from CommConfigIdType a ");
    ...
    public int getMaxSeqNo(){
		int temp = 1;
		try {
			List<?> l = getHibernateTemplate().find(" select max(aa.seqNo) from CommConfigIdType aa ");
			if(l != null && l.size() > 0){
				Object o = l.get(0);
				if(o==null){
					return temp;
