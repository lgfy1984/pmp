...
public class CommConfigStaffChargeDAO extends HibernateDaoSupport implements  ICommConfigStaffChargeDAO{

	private static final Logger log = LogManager.getLogger(CommConfigStaffChargeDAO.class);
	
	public CommConfigStaffChargeType findById(String id){
		try{
			CommConfigStaffChargeType fac = null;
			List<?> ls = getHibernateTemplate().find(" from CommConfigStaffChargeType a where a.itemCode = ? ", id);
			if(ls != null && ls.size() >0){
				fac = (CommConfigStaffChargeType) ls.get(0);
			}
	...
	public CommConfigStaffChargeType findByItemName(String itemName){
		try{
			CommConfigStaffChargeType fac = null;
			List<?> ls = getHibernateTemplate().find(" from CommConfigStaffChargeType a where a.itemName = ? ", itemName);
			if(ls != null && ls.size() >0){
				fac = (CommConfigStaffChargeType) ls.get(0);
			}
			log.debug("findById success!");
	...
	public String getItemName(String id){
		String itemName = "";
		try {
			List<?> l = getHibernateTemplate().find(" from CommConfigStaffChargeType aa where aa.itemCode = ? ", id);
			if(l != null && l.size() > 0){
				CommConfigStaffChargeType pub = (CommConfigStaffChargeType)l.get(0);
    ...
    public void save(CommConfigStaffChargeType CommConfigStaffChargeType){
    	try {
        	getHibernateTemplate().save(CommConfigStaffChargeType);
    ...
    public void update(CommConfigStaffChargeType CommConfigStaffChargeType){
    	try {
        	getHibernateTemplate().update(CommConfigStaffChargeType);
    ...
    public void delete(CommConfigStaffChargeType CommConfigStaffChargeType){
    	try {
        	getHibernateTemplate().delete(CommConfigStaffChargeType);
    ...
    public List<?> findAll(){
    	try {
    		String sql = " from CommConfigStaffChargeType a  ";
    ...
    public List<?> getData(String itemCode, String itemName, String inputCode, String seqNo, String orderNo, int curCount,int pageSize){
    	try {
    		String sql = " select a.itemCode, a.itemName,a.inputCode,a.comments,a.seqNo,a.itemCost,a.itemUnit  ";
	    	sql += " from CommConfigStaffChargeType a  where 1=1 ";
    ...
    public int getMaxSeqNo(){
		int temp = 1;
		try {
			List<?> l = getHibernateTemplate().find(" select max(aa.seqNo) from CommConfigStaffChargeType aa ");
			if(l != null && l.size() > 0){
				Object o = l.get(0);
				if(o==null){
					return temp;
