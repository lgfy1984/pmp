...
public class CommConfigVipDAO extends HibernateDaoSupport implements  ICommConfigVipDAO{

	private static final Logger log = LogManager.getLogger(CommConfigVipDAO.class);
	
	public CommConfigVip findById(String id){
		try{
			CommConfigVip fac = null;
			List<?> ls = getHibernateTemplate().find(" from CommConfigVip a where a.itemCode = ? ", id);
			if(ls != null && ls.size() >0){
				fac = (CommConfigVip) ls.get(0);
			}
	...
	public String getItemName(String id){
		String itemName = "";
		try {
			List<?> l = getHibernateTemplate().find(" from CommConfigVip aa where aa.itemCode = ? ", id);
			if(l != null && l.size() > 0){
				CommConfigVip pub = (CommConfigVip)l.get(0);
    ...
    public void save(CommConfigVip commConfigCountry){
    	try {
    ...
    public void update(CommConfigVip commConfigCountry){
    	try {
    ...
    public void delete(CommConfigVip commConfigCountry){
    	try {
    ...
    public List<?> getData(String itemCode, String itemName, String inputCode, String seqNo, String orderNo, int curCount,int pageSize){
    	try {
	    	...
	    	sql += " from CommConfigVip a  where 1=1 ";
    ...
    public int getMaxSeqNo(){
		int temp = 1;
		try {
			List<?> l = getHibernateTemplate().find(" select max(aa.seqNo) from CommConfigVip aa ");
			if(l != null && l.size() > 0){
				Object o = l.get(0);
				if(o==null){
					return temp;
