...
public class CommConfigEconkindDAO extends HibernateDaoSupport implements  ICommConfigEconkindDAO{

	private static final Logger log = LogManager.getLogger(CommConfigEconkindDAO.class);
	
	public CommConfigEconkind findById(String id){
		try{
			CommConfigEconkind fac = null;
			List<?> ls = getHibernateTemplate().find(" from CommConfigEconkind a where a.itemCode = ? ", id);
			if(ls != null && ls.size() >0){
				fac = (CommConfigEconkind) ls.get(0);
			}
	...
	public String getItemName(String id){
		String itemName = "";
		try {
			List<?> l = getHibernateTemplate().find(" from CommConfigEconkind aa where aa.itemCode = ? ", id);
			if(l != null && l.size() > 0){
				CommConfigEconkind pub = (CommConfigEconkind)l.get(0);
    ...
    public void save(CommConfigEconkind commConfigEconkind){
    	try {
        	getHibernateTemplate().save(commConfigEconkind);
    ...
    public void update(CommConfigEconkind commConfigEconkind){
    	try {
        	getHibernateTemplate().update(commConfigEconkind);
    ...
    public void delete(CommConfigEconkind commConfigEconkind){
    	try {
        	getHibernateTemplate().delete(commConfigEconkind);
    ...
    public List<?> findAll(){
    	try {
    		String sql = " from CommConfigEconkind a  ";
    ...
    public List<?> getData(String itemCode, String itemName, String inputCode, String seqNo, String orderNo, int curCount,int pageSize){
    	try {
	    	...
	    	sql .append( " from CommConfigEconkind a ");
    ...
    public int getMaxSeqNo(){
		int temp = 1;
		try {
			List<?> l = getHibernateTemplate().find(" select max(aa.seqNo) from CommConfigEconkind aa ");
			if(l != null && l.size() > 0){
				Object o = l.get(0);
				if(o==null){
					return temp;
