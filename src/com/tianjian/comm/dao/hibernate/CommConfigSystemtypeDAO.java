...
public class CommConfigSystemtypeDAO extends HibernateDaoSupport implements  ICommConfigSystemtypeDAO{

	private static final Logger log = LogManager.getLogger(CommConfigSystemtypeDAO.class);
	
	public CommConfigSystemtype findById(String id){
		try{
			CommConfigSystemtype fac = null;
			List<?> ls = getHibernateTemplate().find(" from CommConfigSystemtype a where a.itemCode = ? ", id);
			if(ls != null && ls.size() >0){
				fac = (CommConfigSystemtype) ls.get(0);
			}
	...
	public String getItemName(String id){
		String itemName = "";
		try {
			List<?> l = getHibernateTemplate().find(" from CommConfigSystemtype aa where aa.itemCode = ? ", id);
			if(l != null && l.size() > 0){
				CommConfigSystemtype pub = (CommConfigSystemtype)l.get(0);
    ...
    public void save(CommConfigSystemtype commConfigCountry){
    	try {
    ...
    public void update(CommConfigSystemtype commConfigCountry){
    	try {
    ...
    public void delete(CommConfigSystemtype commConfigCountry){
    	try {
    ...
    public List<?> findAll(){
    	try {
    		String sql = " from CommConfigSystemtype a  ";
    ...
    public List<?> getData(String itemCode, String itemName, String inputCode, String seqNo, String orderNo, int curCount,int pageSize){
    	try {
	    	...
	    	sql += " from CommConfigSystemtype a  where 1=1 ";
