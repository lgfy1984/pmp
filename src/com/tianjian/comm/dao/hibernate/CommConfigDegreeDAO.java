...
public class CommConfigDegreeDAO extends HibernateDaoSupport implements  ICommConfigDegreeDAO{

	private static final Logger log = LogManager.getLogger(CommConfigDegreeDAO.class);
	
	public CommConfigDegree findById(String id){
		try{
			CommConfigDegree fac = null;
			List<?> ls = getHibernateTemplate().find(" from CommConfigDegree a where a.itemCode = ? ", id);
			if(ls != null && ls.size() >0){
				fac = (CommConfigDegree) ls.get(0);
			}
	...
	public String getItemName(String id){
		String itemName = "";
		try {
			List<?> l = getHibernateTemplate().find(" from CommConfigDegree aa where aa.itemCode = ? ", id);
			if(l != null && l.size() > 0){
				CommConfigDegree pub = (CommConfigDegree)l.get(0);
    ...
    public void save(CommConfigDegree commConfigCountry){
    	try {
    ...
    public void update(CommConfigDegree commConfigCountry){
    	try {
    ...
    public void delete(CommConfigDegree commConfigCountry){
    	try {
    ...
    public List<?> findAll(){
    	try {
    		String sql = " from CommConfigDegree a  ";
    ...
    public List<?> getData(String itemCode, String itemName, String inputCode, String seqNo, String orderNo, int curCount,int pageSize){
    	try {
	    	...
	    	sql .append( " from CommConfigDegree a");
    ...
    public int getMaxSeqNo(){
		int temp = 1;
		try {
			List<?> l = getHibernateTemplate().find(" select max(aa.seqNo) from CommConfigDegree aa ");
			if(l != null && l.size() > 0){
				Object o = l.get(0);
				if(o==null){
					return temp;
