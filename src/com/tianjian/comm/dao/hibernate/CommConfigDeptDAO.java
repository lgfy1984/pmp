...
public class CommConfigDeptDAO extends HibernateDaoSupport implements  ICommConfigDeptDAO{

	private static final Logger log = LogManager.getLogger(CommConfigDeptDAO.class);
	
	public CommConfigDept findById(String id){
		try{
			CommConfigDept fac = null;
			List<?> ls = getHibernateTemplate().find(" from CommConfigDept a where a.itemCode = ? ", id);
			if(ls != null && ls.size() >0){
				fac = (CommConfigDept) ls.get(0);
			}
	...
	public String getItemName(String id){
		String itemName = "";
		try {
			List<?> l = getHibernateTemplate().find(" from CommConfigDept aa where aa.itemCode = ? ", id);
			if(l != null && l.size() > 0){
				CommConfigDept pub = (CommConfigDept)l.get(0);
    ...
    public void save(CommConfigDept commConfigDept){
    	try {
        	getHibernateTemplate().save(commConfigDept);
    ...
    public void update(CommConfigDept commConfigDept){
    	try {
        	getHibernateTemplate().update(commConfigDept);
    ...
    public void delete(CommConfigDept commConfigDept){
    	try {
        	getHibernateTemplate().delete(commConfigDept);
    ...
    public List<?> findAll(){
    	try {
    		String sql = " from CommConfigDept a  ";
    ...
    public List<?> getData(String itemCode, String itemName, String inputCode, String seqNo, String orderNo, int curCount,int pageSize){
    	try {
	    	...
	    	sql .append( " from CommConfigDept a");
    ...
    public int getMaxSeqNo(){
		int temp = 1;
		try {
			List<?> l = getHibernateTemplate().find(" select max(aa.seqNo) from CommConfigDept aa ");
			if(l != null && l.size() > 0){
				Object o = l.get(0);
				if(o==null){
					return temp;
