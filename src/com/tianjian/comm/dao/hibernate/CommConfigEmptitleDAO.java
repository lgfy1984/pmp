...
public class CommConfigEmptitleDAO extends HibernateDaoSupport implements  ICommConfigEmptitleDAO{

	private static final Logger log = LogManager.getLogger(CommConfigEmptitleDAO.class);
	
	public CommConfigEmptitle findById(String id){
		try{
			CommConfigEmptitle fac = null;
			List<?> ls = getHibernateTemplate().find(" from CommConfigEmptitle a where a.itemCode = ? ", id);
			if(ls != null && ls.size() >0){
				fac = (CommConfigEmptitle) ls.get(0);
			}
	...
	public String getItemName(String id){
		String itemName = "";
		try {
			List<?> l = getHibernateTemplate().find(" from CommConfigEmptitle aa where aa.itemCode = ? ", id);
			if(l != null && l.size() > 0){
				CommConfigEmptitle pub = (CommConfigEmptitle)l.get(0);
    ...
    public void save(CommConfigEmptitle commConfigCountry){
    	try {
    ...
    public void update(CommConfigEmptitle commConfigCountry){
    	try {
    ...
    public void delete(CommConfigEmptitle commConfigCountry){
    	try {
    ...
    public List<?> findAll(){
    	try {
    		String sql = " from CommConfigEmptitle a  ";
    ...
    public List<?> getData(String itemCode, String itemName, String inputCode, String seqNo, String orderNo, int curCount,int pageSize){
    	try {
	    	...
	    	sql .append( " from CommConfigEmptitle a ");
    ...
    public int getMaxSeqNo(){
		int temp = 1;
		try {
			List<?> l = getHibernateTemplate().find(" select max(aa.seqNo) from CommConfigEmptitle aa ");
			if(l != null && l.size() > 0){
				Object o = l.get(0);
				if(o==null){
					return temp;
