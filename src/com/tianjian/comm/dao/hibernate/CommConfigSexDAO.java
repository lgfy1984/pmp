...
public class CommConfigSexDAO extends HibernateDaoSupport implements  ICommConfigSexDAO{

	private static final Logger log = LogManager.getLogger(CommConfigSexDAO.class);
	
	public CommConfigSex findById(String id){
		try{
			CommConfigSex fac = null;
			List<?> ls = getHibernateTemplate().find(" from CommConfigSex a where a.itemCode = ? ", id);
			if(ls != null && ls.size() >0){
				fac = (CommConfigSex) ls.get(0);
			}
	...
	public String getItemName(String id){
		String itemName = "";
		try {
			List<?> l = getHibernateTemplate().find(" from CommConfigSex aa where aa.itemCode = ? ", id);
			if(l != null && l.size() > 0){
				CommConfigSex pub = (CommConfigSex)l.get(0);
    ...
    public void save(CommConfigSex commConfigCountry){
    	try {
    ...
    public void update(CommConfigSex commConfigCountry){
    	try {
    ...
    public void delete(CommConfigSex commConfigCountry){
    	try {
    ...
    public List<?> findAll(){
    	try {
    		String sql = " from CommConfigSex a  ";
    ...
    public List<?> getData(String itemCode, String itemName, String inputCode, String seqNo, String orderNo, int curCount,int pageSize){
    	try {
	    	...
	    	sql += " from CommConfigSex a  where 1=1 ";
    ...
    public int getMaxSeqNo(){
		int temp = 1;
		try {
			List<?> l = getHibernateTemplate().find(" select max(aa.seqNo) from CommConfigSex aa ");
			if(l != null && l.size() > 0){
				Object o = l.get(0);
				if(o==null){
					return temp;
