...
//删除
function cmdDel(id) {    
    if (!confirm(getCommMessage("<bean:message key="comm.jsp.different.text23" bundle="conf.comm.comm"/>?","",""))) return false;    
...
<body onload="showCommMessage('','<%=data.getMessage() %>','1')" >
		...
		<form name="form" method="post" action="<%=request.getContextPath()%>/comm/commConfigGbDrugclass.do">
			...
			<table border="0" cellpadding="0" cellspacing="0" class="tblSearch" align="center">
				<tr>
					<td class="tblTitle">
						<bean:message key="comm.jsp.common.text51" bundle="conf.comm.comm"/>
					</td>
				</tr>
				<tr>
					<td >
						...
						<input type="button"  class="btnSave" name="button" value="查&nbsp;&nbsp询" onClick="submitQueryForm();" />
				    </td>
				</tr>
			</table>
			<!--列表标题-->
			   ...
			   <caption style="text-align:left;">
			    ...
			    <thead>
			    	<tr class="lstName">
			            ...
			            <th width="15%" height="26"><bean:message key="comm.jsp.common.text28" bundle="conf.comm.comm"/><div><img src="include/images/cmdOrderByAsc.gif" alt="Ascender" vspace="7" onclick="commandOrderBy('2', '0')" /><img border="0" onclick="commandOrderBy('2', '1')" alt="Descender" src="include/images/cmdOrderByDesc.gif" /></div></th>
			            ...
			            <th width="4%" height="26"><bean:message key="comm.jsp.common.text25" bundle="conf.comm.comm"/><div><img src="include/images/cmdOrderByAsc.gif" alt="Ascender" vspace="7" onclick="commandOrderBy('4', '0')" /><img border="0" onclick="commandOrderBy('4', '1')" alt="Descender" src="include/images/cmdOrderByDesc.gif" /></div></th>
			            <th width="4%" height="26"><bean:message key="comm.jsp.common.text20" bundle="conf.comm.comm"/><div><img src="include/images/cmdOrderByAsc.gif" alt="Ascender" vspace="7" onclick="commandOrderBy('5', '0')" /><img border="0" onclick="commandOrderBy('5', '1')" alt="Descender" src="include/images/cmdOrderByDesc.gif" /></div></th>
			            <th width="4%" height="26"><bean:message key="comm.jsp.common.text21" bundle="conf.comm.comm"/><div><img src="include/images/cmdOrderByAsc.gif" alt="Ascender" vspace="7" onclick="commandOrderBy('6', '0')" /><img border="0" onclick="commandOrderBy('6', '1')" alt="Descender" src="include/images/cmdOrderByDesc.gif" /></div></th>
					...
					%>
						<tr class="list_neirong" >
							...
							<td style="text-align:center" >
								&nbsp;<%=data.getLevelFlagList()[i]%>
							</td>
							<td style="text-align:center" >
								&nbsp;<%=data.getParentIdList()[i]%>
							</td>
							<td style="text-align:left" >
								&nbsp;<%=data.getParentNameList()[i]%>
							</td>
			...
			<table  width="100%" align="center" class="tblScrollFooter" <c:if test="${pb.count<=0}">style="display:none"</c:if>>
				<tr>
					<td colspan="9" align="center"  class="footer" >
						...
						%>
						<input type="hidden" id="pageId" name="pageId" value="page_282881de3903c90c013903c90c270000" />
						<input type="hidden" id="reHref" name="reHref" value="<%=request.getContextPath()%>/comm/commConfigGbDrugclass.do?verbId=query" />
