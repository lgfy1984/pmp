...
<script type="text/javascript"	src="${path }/style/easyui/jquery.easyui.min.js"></script>
		...
		function saveForm(){
			...
			if(isNaN(document.form.seqNo.value)){
				$.messager.alert('提示','<bean:message key="comm.jsp.commconfigyes.update.saveForm1" bundle="conf.comm.CommMessageguoh"/>'); 
				return true;
			}
			if(document.form.itemCode.value == ""){
			 	$.messager.alert('提示','<bean:message key="comm.jsp.commconfigyes.update.saveForm2" bundle="conf.comm.CommMessageguoh"/>');
			 	return ;
			}
			if(document.form.itemName.value == ""){
			 	$.messager.alert('提示','<bean:message key="comm.jsp.commconfigyes.update.saveForm3" bundle="conf.comm.CommMessageguoh"/>');
			 	return ;
			}
			//修改
	...
	<body onload="showHspMessage('<%=commConfigYes.getMessage()%>')">
		<form name="form" method="post" action="comm/commConfigYes.do">
			...
			<!-- <table border="0" cellpadding="0" cellspacing="0" class="tblFill"
				border="0" cellpadding="0" cellspacing="0" class="tblFill">
				<tr>
					<td class="tblTitle" colspan="4"><span>※</span>
					<bean:message key="comm.jsp.commconfigyes.update.td1"
							bundle="conf.comm.CommMessageguoh" /><span>※</span>
					</td>
				</tr>
				<tr>
	
					<td class="tblLable"><span>*</span> <bean:message
							key="comm.jsp.commconfigyes.update.td3"
							bundle="conf.comm.CommMessageguoh" />:</td>
					<td><input type="text" name="itemCode" size="20" maxlength="9"
						onkeypress="eventOnKeyPress('itemName')"
						value="<%=commConfigYes.getItemCode() %>" readonly /></td>
					<td class="tblLable"><span>*</span> <bean:message
							key="comm.jsp.commconfigyes.update.td4"
							bundle="conf.comm.CommMessageguoh" />:</td>
					<td><input type="text" name="itemName" size="50" maxlength="50"
						onkeypress="eventOnKeyPress('seqNo')" max="32" dataType="LimitB"
						msg="名称输入过长" value="<%=commConfigYes.getItemName() %>" /></td>
				</tr>
				<tr>
	
					<td class="tblLable"><bean:message
							key="comm.jsp.commconfigyes.update.td2"
							bundle="conf.comm.CommMessageguoh" />:</td>
					<td><input type="text" name="seqNo" id="seqNo" size="20"
						maxlength="6" onkeypress="eventOnKeyPress('comments')" max="11"
						dataType="LimitB" msg="序号输入过长"
						value="<%=commConfigYes.getSeqNo() %>" /></td>
					<td class="tblLable"><bean:message
							key="comm.jsp.commconfigyes.update.td5"
							bundle="conf.comm.CommMessageguoh" />:</td>
					<td><input type="text" name="comments" id="comments" size="50"
						maxlength="50" onkeypress="eventOnKeyPress('btnSaveForm')" max="40"
						dataType="LimitB" msg="备注输入过长"
						value="<%=commConfigYes.getComments() %>" /></td>
				</tr>
			</table> -->
				...
				<table class='crm_panel_table'>
					<tr>
						...
						<td class='crm_edit_item_content'><input class="text" onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
							...
							dataType="LimitB" msg="代码输入过长" value="<%=commConfigYes.getItemCode()%>"
							readonly="readonly" /></td>
						<td class='crm_edit_item_name'><label class="redlable">*</label>名称</td>
						<td class='crm_edit_item_content'><input class="text" onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
								onfocus="fEvent('focus',this)" onmouseout="fEvent('mouseout',this)" 
							type="text" class="kuandu" name="itemName" size="20" maxlength="9"
							onkeypress="eventOnKeyPress('seqNo')" max="40" dataType="LimitB" msg="名称输入过长" value="<%=commConfigYes.getItemName()%>" />
						</td>
					</tr>
					<tr>
						...
						<td class='crm_edit_item_content' colspan="3"><input onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
							...
							value="<%=commConfigYes.getComments()%>" max="40" dataType="LimitB"
