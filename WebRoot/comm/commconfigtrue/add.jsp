		...
		<form name="form" method="post" action="comm/commConfigTrue.do">
			...
			<!-- <table border="0" cellpadding="0" cellspacing="0" class="tblFill"
				...
				<tr>
					...
					<bean:message key="comm.jsp.commconfigtrue.add.td1"
							bundle="conf.comm.CommMessageguoh" /> <span>※</span>
					</td>
				</tr>
				<tr>
					<td class="tblLable"><span>*</span> <bean:message
							key="comm.jsp.commconfigtrue.add.td3"
							bundle="conf.comm.CommMessageguoh" />:</td>
					<td><input type="text" name="itemCode" size="20" maxlength="2"
						onkeypress="eventOnKeyPress('itemName')"
						onkeyup="value=value.replace(/[^\d\w]/g,'')" max="32"
						dataType="LimitB" msg="代码输入过长"
						value="<%=commConfigTrue.getItemCode() %>" /></td>
					<td class="tblLable"><span>*</span> <bean:message
							key="comm.jsp.commconfigtrue.add.td4"
							bundle="conf.comm.CommMessageguoh" />:</td>
					<td><input type="text" name="itemName" size="50" maxlength="50"
						onkeypress="eventOnKeyPress('seqNo')"
						onkeyup="value=value.replace(/[^\d\w]/g,'')" max="40"
						dataType="LimitB" msg="名称输入过长"
						value="<%=commConfigTrue.getItemName() %>" /></td>
				</tr>
				<tr>
	
					<td class="tblLable"><bean:message
							key="comm.jsp.commconfigtrue.add.td2"
							bundle="conf.comm.CommMessageguoh" />:</td>
					<td><input type="text" name="seqNo" id="seqNo" size="30"
						maxlength="20" onkeypress="eventOnKeyPress('comments')" max="11"
						dataType="LimitB" msg="序号输入过长"
						value="<%=commConfigTrue.getSeqNo() %>" /></td>
					<td class="tblLable"><bean:message
							key="comm.jsp.commconfigtrue.add.td5"
							bundle="conf.comm.CommMessageguoh" />:</td>
					<td><input type="text" name="comments" id="comments" size="30"
						maxlength="50" onkeypress="eventOnKeyPress('btnSaveForm')" max="40"
						dataType="LimitB" msg="备注输入过长"
						value="<%=commConfigTrue.getComments() %>" /></td>
				</tr>
			</table> -->
