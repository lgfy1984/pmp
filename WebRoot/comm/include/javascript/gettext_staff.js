	...
	function add(url,itemName,drugDictId,drugCode,minSpec,minUnits,divlevel){	
			...
			*/
			var check=path.substring((path.length-9),path.length);
				document.getElementById("div_pinyin").innerHTML = '<div class="writediv"><select id="inputTypeId" style="width:50px;margin-right:10px;"><option value="1">拼音码</option><option value="2">代码</option><option value="3">汉字</option></select>查询条件：<input type="hidden" id="hidden_value" value="" /><input type="hidden" id="drugCode" value="" /><input type="hidden" id="minSpec" value="" /><input type="hidden" id="minUnits" value="" /><input type="text" name="divInputName" id="input_value" class="write_input" value="" onkeyup="findNames(\''+url+'\',\''+itemName+'\',\''+drugDictId+'\',\''+drugCode+'\',\''+minSpec+'\',\''+minUnits+'\');" maxlength="30" /><input type="button" class="button_blue1_s0" value="选中" onclick="xuanz(\''+itemName+'\',\''+drugDictId+'\',\''+drugCode+'\',\''+minSpec+'\',\''+minUnits+'\');" /></div><div id="guoji_div"><table id="guoji_table" border="0" cellspacing="1" cellpadding="0"><tr class="pinyin_nav"><td width="80">代码</td><td width="90">拼音码</td><td width="166">名称</td><td width="166">最小规格</td><td width="166">最小单位</td></tr><tbody id="guoji_body"></tbody></table></div>';		
