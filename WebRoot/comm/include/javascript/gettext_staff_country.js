	...
	调用ajax
	*/
    function findNames(url,itemCode,itemName) {
        ...
        default :    
			...
			*/
	        ...
	        if(completeBody.rows.length>0){
	            ...
	            XMLHttpReq.onreadystatechange = function(){processMatchResponse(itemCode,itemName)};  //指定对象状态判断的方法
	...
	//对象状态判断的方法
    function processMatchResponse(itemCode,itemName) {
    	
    	if (XMLHttpReq.readyState == 4) { // readyState　为正常返回的时候
        	if (XMLHttpReq.status == 200) {　　//成功响应 
        		     
				  			
                setNames(XMLHttpReq.responseXML,itemName,itemCode); 
	...
	*/
     function setNames(names,itemName,itemCode) {            
       ...
       　　　　　　　　　　　　　　　　　　　　//设置显示位置
        ...
        for(var i=0 ;i < elementroot.length;i++){
			...
			document.getElementById(itemCode).value = this.getElementsByTagName('td')[2].innerHTML;
			document.getElementById(itemName).value = this.getElementsByTagName('td')[0].innerHTML;
...
选中处理
*/
	function xuanz(itemCode,itemName){
		if(document.getElementById("input_value").value != "" && currentLine != -1){
			document.getElementById(itemCode).value = document.getElementById("input_value").value;
			document.getElementById(itemName).value = document.getElementById("hidden_value").value;
		}		
	    ...
	    switch (e.keyCode){ 
	        ...
	        case 13:
	        	if(windowstate == true){
			        ...
			        document.getElementById(item_inputcode).value = completeBody.rows[currentLine].getElementsByTagName("td")[0].innerHTML;
	...
	*****/
	function add(url,itemCode,itemName,divlevel){	
			path=url;
			if(divlevel != null){
			...
			*/
			var check=path.substring((path.length-9),path.length);
				document.getElementById("div_pinyin").innerHTML = '<div class="writediv"><select id="inputTypeId" style="width:50px;margin-right:10px;"><option value="1">拼音码</option><option value="2">代码</option><option value="3">汉字</option></select>查询条件：<input type="hidden" id="hidden_value" value="" /><input type="text" name="divInputName" id="input_value" class="write_input" value="" onkeyup="findNames(\''+url+'\',\''+itemCode+'\',\''+itemName+'\');" maxlength="30" /><input type="button" class="button_blue1_s0" value="选中" onclick="xuanz(\''+itemCode+'\',\''+itemName+'\');" /></div><div id="guoji_div"><table id="guoji_table" border="0" cellspacing="1" cellpadding="0"><tr class="pinyin_nav"><td width="80">代码</td><td width="90">拼音码</td><td width="166">名称</td></tr><tbody id="guoji_body"></tbody></table></div>';		
			//fade("body_div");//调用该函数作用是背景渐变透明	
			
			findNames(url,itemCode,itemName);
