...
<script type="text/javascript"	src="${path }/style/easyui/jquery.easyui.min.js"></script>
		...
		function saveForm(){
			...
			if(isNaN(document.form.itemCode.value)){
				$.messager.alert('提示','<bean:message key="comm.jsp.common.warn1" bundle="conf.comm.comm"/>'); 
				return true;
			}
			...
			<!-- <table border="0" cellpadding="0" cellspacing="0" class="tblFill"
				...
				<tr>
					<td class="tblTitle" colspan="4"><span>※</span> <bean:message
							key="comm.jsp.different.text41" bundle="conf.comm.comm" /> <span>※</span>
					</td>
				</tr>
				<tr>
					...
					<td><input type="text" name="itemCode" size="20" maxlength="2"
						...
						msg="代码输入过长" value="<%=commConfigHave.getItemCode() %>" /></td>
					<td class="tblLable"><span>*</span> <bean:message
				...
				<tr>
					...
					<td><input type="text" name="seqNo" id="principal_phone"
						size="30" maxlength="20" onkeypress="eventOnKeyPress('comments')"
						max="11" dataType="LimitB" msg="序号输入过长"
						value="<%=commConfigHave.getSeqNo() %>" /></td>
					<td class="tblLable"><bean:message key="comm.jsp.common.text4"
							bundle="conf.comm.comm" />:</td>
					<td><input type="text" name="comments" id="principal" size="30"
						maxlength="50" onkeypress="eventOnKeyPress('btnSaveForm')" max="40"
						dataType="LimitB" msg="备注输入过长"
						value="<%=commConfigHave.getComments() %>" /></td>
				</tr>
			</table> -->
