...
<jsp:useBean id="commConfigHave" scope="request" type="com.tianjian.comm.struts.form.CommConfigHaveForm" />
  ...
  <script type="text/javascript" src="${path}/js/default.js"></script>
	...
	<body onload="showHspMessage('<%=commConfigHave.getMessage()%>')">
		...
		<form name="form" method="post" action="comm/commConfigHave.do">
			<!-- Head line -->
			<input type="hidden" name="verbId" value="<%=commConfigHave.getVerbId() %>" />
			<input type="hidden" name="orderNo" value="<%=commConfigHave.getOrderNo() %>" />
			<input type="hidden" name="asc" value="<%=commConfigHave.getAsc() %>" />
			<input type="hidden" name="itemCodeHidden" value="<%=commConfigHave.getItemCodeHidden() %>" />
				...
				<div class="crm_input_item">
					...
					<input class="crm_search_input_text"
						name="inputCode" onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
							...
							type="text" value="<%=commConfigHave.getInputCode()%>" />
				</div>
				<div class="crm_input_item">
			...
			<table style="table-layout: fixed;" class='crm_table_content'>
				...
				<%if (commConfigHave.getItemCodeList() != null && commConfigHave.getItemCodeList().length > 0) {
					for (int i = 0; i < commConfigHave.getItemCodeList().length; i++) {
					if(i%2==0){%>
									<tr class="odd">
								<%}else{ %>
									<tr class="even">
				...
				%>
					...
					<td>
						<%=commConfigHave.getItemCodeList()[i]%>
					</td>
					<td>
						<%=commConfigHave.getItemNameList()[i]%>
					</td>
					<td>
						<%=commConfigHave.getSeqNoList()[i]%>
					</td>
					<td>
						<%=commConfigHave.getInputCodeList()[i]%>
					</td>							
					
					<td style="cursor:pointer;"><input type="button" class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'" value="详细"  onclick="cmdView('<%=commConfigHave.getItemCodeList()[i] %>')"/></td>
						<td style="cursor:pointer;"><input type="button" class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'" value="修改"  onclick="cmdEdit('<%=commConfigHave.getItemCodeList()[i] %>')"/></td>
						<td style="cursor:pointer;"><input type="button" class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'" value="删除"  onclick="cmdDel('<%=commConfigHave.getItemCodeList()[i] %>')"/></td>				
				</tr>
				<%
						}
					}
