...
<jsp:useBean id="commConfigEmptitle" scope="request" type="com.tianjian.comm.struts.form.CommConfigEmptitleForm" />
...
<html>
	<head>
		...
		%>
		<title><bean:message  bundle="comm.commLocale" key="comm.jsp.commconfigemptitlelist.text70"/></title>
...
<link rel="stylesheet" type="text/css" href="${path}/style/easyuiUpdate.css">
	...
	<body>
		...
		<form name="form" method="post" action="comm/commConfigEmptitle.do">
			<!-- Head line -->
			<input type="hidden" name="verbId" value="<%=commConfigEmptitle.getVerbId()%>"/>
			<input type="hidden" name="orderNo" value="<%=commConfigEmptitle.getOrderNo()%>"/>
			<input type="hidden" name="asc" value="<%=commConfigEmptitle.getAsc()%>"/>
		...
		<div class='crm_search_div' style='overflow: hidden'>
		 <div class="crm_input_item">
		  	...
		  	<input class="crm_search_input_text" name="itemCode" type="text" onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
								onfocus="fEvent('focus',this)" onmouseout="fEvent('mouseout',this)" onkeypress="eventOnKeyPress('itemName')" value="<%=commConfigEmptitle.getItemCode()%>" />
		 </div>
		 <div class="crm_input_item">
		  	<span>名称</span>
		  	<input class="crm_search_input_text" name="itemName" type="text" onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
								onfocus="fEvent('focus',this)" onmouseout="fEvent('mouseout',this)" onkeypress="eventOnKeyPress('inputCode')" value="<%=commConfigEmptitle.getItemName() %>" />
		 </div>
		 <div class="crm_input_item">
		  	<span>输入码</span>
		  	<input class="crm_search_input_text" name="inputCode" type="text" onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
								onfocus="fEvent('focus',this)" onmouseout="fEvent('mouseout',this)" onkeypress="eventOnKeyPress('btnSave')" value="<%=commConfigEmptitle.getInputCode() %>" />
		 </div>
		 <div class="crm_input_item">
	  ...
	  <div class='crm_table_out'>
		  ...
		  <tbody>
			...
			<%if (commConfigEmptitle.getItemCodeList() != null && commConfigEmptitle.getItemCodeList().length > 0) {
				for (int i = 0; i < commConfigEmptitle.getItemCodeList().length; i++) {
					if(i%2==0){%>
						<tr class="odd">
					<%}else{ %>
						<tr class="even">
					<%}
					%>
					<td><%=commConfigEmptitle.getItemCodeList()[i]%></td>
					<td><%=commConfigEmptitle.getItemNameList()[i]%></td>
					<td><%=commConfigEmptitle.getSeqNoList()[i]%></td>
					<td><%=commConfigEmptitle.getInputCodeList()[i]%></td>
					<td style="cursor:pointer;"><input type="button" class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'" value="详细"  onclick="cmdView('<%=commConfigEmptitle.getItemCodeList()[i] %>')"/></td>
					<td style="cursor:pointer;"><input type="button" class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'" value="修改"  onclick="cmdEdit('<%=commConfigEmptitle.getItemCodeList()[i] %>')"/></td>
					<td style="cursor:pointer;"><input type="button" class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'" value="删除"  onclick="cmdDel('<%=commConfigEmptitle.getItemCodeList()[i] %>')"/></td>
				  </tr>
				<%}
			}
			%>
