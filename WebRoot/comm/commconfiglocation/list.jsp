				...
				<tbody id="interval_row_id">
				  <c:if test="${pb.count<=0}">
					...
					<tr class="list_neirong">
						...
						<td>
							<img onClick="cmdEdit('<%=commConfigLocation.getIdList()[i]%>')" alt="<bean:message key="comm.jsp.common.text47" bundle="conf.comm.comm"/>" src="include/images/cmdEdit_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
						<td>
							<img onClick="cmdDel('<%=commConfigLocation.getIdList()[i]%>')" alt="<bean:message key="comm.jsp.common.text35" bundle="conf.comm.comm"/>" src="include/images/cmdDel_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
					</tr>
					<%
						}
			...
			<table width="100%" align="center" class="tblScrollFooter" <c:if test="${pb.count<=0}">style="display:none"</c:if>>
				<tr>
					<td colspan="11" align="center" class="footer">
						...
						<input type="hidden" id="reHref" name="reHref" value="<%=request.getContextPath()%>/comm/commConfigLocation.do?verbId=query" />
