...
<jsp:useBean id="data" scope="request" type="com.tianjian.comm.struts.form.CommIHEFacilityForm" />
...
<html>
	<head>
		...
		%>
		<title><bean:message  bundle="comm.comIHE" key="comm.jsp.commIHEFacility.list"/></title>
			...
			<table width="95%" class="table2" align="center" border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td class="list_biaoti">
						<bean:message  bundle="comm.comIHE" key="comm.jsp.commIHEFacility.list"/>
					</td>
					<td id="list_C"></td>
					<td class="list_cc">
						<bean:message  bundle="comm.comIHE" key="comm.jsp.segment10"/>:
						<input name="seqNo" type="text" onkeypress="eventOnKeyPress('facilityUniversalId')" value="<%=data.getSeqNo()%>" style="height:17px;">
					</td>
					<td class="list_cc">
						<bean:message  bundle="comm.comIHE" key="comm.jsp.segment11"/>:
						<input name="facilityUniversalId" type="text" onkeypress="eventOnKeyPress('facilityNamespaceId')" value="<%=data.getFacilityUniversalId()%>" style="height:17px;">
					</td>
					<td class="list_cc">
						<bean:message  bundle="comm.comIHE" key="comm.jsp.segment13"/>:
						<input name="facilityNamespaceId" type="text" value="<%=data.getFacilityNamespaceId()%>" onkeypress="eventOnKeyPress('btnSearch')" style="height:17px;">
					</td>
					<td id="list_R"></td>
					<td class="button">
			...
			<table class="table" align="center" width="95%" cellspacing="1" cellpadding="0">
				...
				<tbody id="interval_row_id">
					...
					<tr class="list_neirong">
						...
						<td class="td_shenglue leftPadding">
							<%=data.getFacilityUniversalIdList()[i]%>
						</td>
						<td class="td_shenglue leftPadding" >
							<%=data.getFacilityUniversalIdTypeList()[i]%>
						</td>
						
						<td class="td_shenglue leftPadding" >
							<%=data.getFacilityNamespaceIdList()[i]%>
						</td>				
						<td>
				...
				<tr <c:if test="${pb.count<=0}">style="display:none"</c:if>>
					<td colspan="8" align="center" bgcolor="#ffffff" height="35px">
						...
						%>
						<input type="hidden" id="pageId" name="pageId" value="page_2828810b369604120136960412480000" />
						<input type="hidden" id="reHref" name="reHref" value="<%=request.getContextPath()%>/comm/commIHEFacility.do?verbId=query" />
