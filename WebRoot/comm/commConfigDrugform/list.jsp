...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body onload="showCommMessage('','<%=data.getMessage() %>','1')">
		<!-- Form中的action项的值必须设置-->
		<form name="form" method="post" action="<%=request.getContextPath()%>/comm/commConfigDrugform.do">
			<!-- Head line -->
			...
			<table  width="100%" align="center" class="tblScrollFooter" <c:if test="${pb.count<=0}">style="display:none"</c:if>> 
				<tr>
					<td colspan="9" align="center"  class="footer" >
						...
						%>
						<input type="hidden" id="pageId" name="pageId" value="page_282881de3903fb5e013903fb5e7e0000" />
						<input type="hidden" id="reHref" name="reHref" value="<%=request.getContextPath()%>/comm/commConfigDrugform.do?verbId=query" />
