...
<jsp:useBean id="commConfigDegree" scope="request" type="com.tianjian.comm.struts.form.CommConfigDegreeForm" />
...
<link rel="stylesheet" type="text/css" href="${path}/style/easyuiUpdate.css">
	...
	<body>
		...
		<form name="form" method="post" action="comm/commConfigDegree.do">
			<!-- Head line -->
			<input type="hidden" name="verbId" value="<%=commConfigDegree.getVerbId()%>">
			<input type="hidden" name="orderNo" value="<%=commConfigDegree.getOrderNo()%>">
			<input type="hidden" name="asc" value="<%=commConfigDegree.getAsc()%>">
			<input type="hidden" name="itemCodeHidden" value="<%=commConfigDegree.getItemCodeHidden()%>">
		<!--zyc--add--b-->
		<div class='crm_search_div' style='overflow: hidden'>
		 <div class="crm_input_item">
		  	...
		  	<input class="crm_search_input_text" name="itemCode" type="text" onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
								onfocus="fEvent('focus',this)" onmouseout="fEvent('mouseout',this)" onkeypress="eventOnKeyPress('itemName')" value="<%=commConfigDegree.getItemCode()%>" />
		 </div>
		 <div class="crm_input_item">
		  	<span>名称</span>
		  	<input class="crm_search_input_text" name="itemName" type="text" onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
								onfocus="fEvent('focus',this)" onmouseout="fEvent('mouseout',this)" onkeypress="eventOnKeyPress('inputCode')" value="<%=commConfigDegree.getItemName() %>" />
		 </div>
		 <div class="crm_input_item">
		  	<span>输入码</span>
		  	<input class="crm_search_input_text" name="inputCode" type="text" onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
								onfocus="fEvent('focus',this)" onmouseout="fEvent('mouseout',this)" onkeypress="eventOnKeyPress('btnSave')" value="<%=commConfigDegree.getInputCode() %>" />
		 </div>
		 <div class="crm_input_item">
	  ...
	  <div class='crm_table_out'>
		  ...
		  <tbody>
			...
			<%if (commConfigDegree.getItemCodeList() != null && commConfigDegree.getItemCodeList().length > 0) {
				for (int i = 0; i < commConfigDegree.getItemCodeList().length; i++) {
					if(i%2==0){%>
						<tr class="odd">
					<%}else{ %>
						<tr class="even">
					<%}
					%>
					<td><%=commConfigDegree.getItemCodeList()[i]%></td>
					<td><%=commConfigDegree.getItemNameList()[i]%></td>
					<td><%=commConfigDegree.getSeqNoList()[i]%></td>
					<td><%=commConfigDegree.getInputCodeList()[i]%></td>
					<td style="cursor:pointer;"><input type="button" class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'" value="详细"  onclick="cmdView('<%=commConfigDegree.getItemCodeList()[i] %>')"/></td>
					<td style="cursor:pointer;"><input type="button" class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'" value="修改"  onclick="cmdEdit('<%=commConfigDegree.getItemCodeList()[i] %>')"/></td>
					<td style="cursor:pointer;"><input type="button" class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'" value="删除"  onclick="cmdDel('<%=commConfigDegree.getItemCodeList()[i] %>')"/></td>
				  </tr>
				<%}
			}
			%>
