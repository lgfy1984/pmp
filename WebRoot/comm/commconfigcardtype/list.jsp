...
<jsp:useBean id="commConfigCardtype" scope="request" type="com.tianjian.comm.struts.form.CommConfigCardtypeForm" />
...
<html>
	<head>
		...
		%>
		<title><bean:message  bundle="comm.commLocale" key="comm.jsp.commconfigcardtypelist.text16"/></title>
...
//修改
function cmdEdit(id,number) {
	if(window.confirm("<bean:message  bundle="comm.commLocale" key="comm.jsp.commconfigcardtypelist.text27"/>")){
   	 	document.form.itemCodeHidden.value = id;  
   	 	document.form.stopFlag.value = number;
...
function isMadeOf(val,str)
{
	...
	<body onload="showMessage('','<%=commConfigCardtype.getMessage()%>','1')">
		<!-- Form中的action项的值必须设置-->
		<form name="form" method="post" action="comm/commConfigCardtype.do">
			<!-- Head line -->
			<input type="hidden" name="verbId" value="<%=commConfigCardtype.getVerbId()%>">
			<input type="hidden" name="orderNo" value="<%=commConfigCardtype.getOrderNo()%>">
			<input type="hidden" name="asc" value="<%=commConfigCardtype.getAsc()%>">
			<input type="hidden" name="itemCodeHidden" value="<%=commConfigCardtype.getItemCodeHidden()%>">
			<input type="hidden" name="stopFlag" value="">
			<!--查询条件-->
			<table width="95%" class="table2" align="center" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td id="list_L"></td>
					<td class="list_biaoti">
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commconfigcardtypelist.text14"/>
					</td>
					<td id="list_C"></td>
					<td class="list_cc">
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.code"/>:
						<input name="itemCode" type="text" onkeypress="eventOnKeyPress('itemName')" value="<%=commConfigCardtype.getItemCode()%>">
					</td>
					<td class="list_cc">
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.name"/>:
						<input name="itemName" type="text" onkeypress="eventOnKeyPress('inputCode')" value="<%=commConfigCardtype.getItemName()%>">
					</td>
					<td class="list_cc">
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.inputCode"/>:
						<input name="inputCode" type="text" onkeypress="eventOnKeyPress('<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.submit"/>')" value="<%=commConfigCardtype.getInputCode()%>">
					</td>
					<td id="list_R"></td>
					<td class="button">
			...
			<table class="table" align="center" width="95%" cellspacing="1" cellpadding="0">
				...
				<tbody id="interval_row_id">
				  <c:if test="${pb.count<=0}">
					...
					<%
							if (commConfigCardtype.getItemCodeList() != null && commConfigCardtype.getItemCodeList().length > 0) {
							for (int i = 0; i < commConfigCardtype.getItemCodeList().length; i++) {
					%>
					<tr class="list_neirong">
						<td>
							<%=commConfigCardtype.getItemCodeList()[i]%>
						</td>
						<td>
							<%=commConfigCardtype.getItemNameList()[i]%>
						</td>
						<td>
							<%=commConfigCardtype.getSeqNoList()[i]%>
						</td>
						<td>
							<%=commConfigCardtype.getInputCodeList()[i]%>
						</td>
						<td>
						<%	
							if(commConfigCardtype.getStopFlagList()[i].equals("0")){ 
						%>
						...
						<td>
							<img onClick="cmdView('<%=commConfigCardtype.getItemCodeList()[i]%>')" alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.detail"/>" src="include/images/cmdView_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
						<td>
							<img onClick="cmdEdit('<%=commConfigCardtype.getItemCodeList()[i]%>','<%=commConfigCardtype.getStopFlagList()[i] %>')" alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.change"/>" src="include/images/cmdEdit_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
						<td>
							<img onClick="cmdDel('<%=commConfigCardtype.getItemCodeList()[i]%>')" alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.delete"/>" src="include/images/cmdDel_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
					</tr>
					<%
						}
