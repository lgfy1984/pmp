...
<jsp:useBean id="commConfigCardorg" scope="request" type="com.tianjian.comm.struts.form.CommConfigCardorgForm" />
...
<html>
	<head>
		...
		%>
		<title><bean:message  bundle="comm.commLocale" key="comm.jsp.commconfigcardorglist.text7"/></title>
...
function isMadeOf(val,str)
{
	...
	<body>
		...
		<form name="form" method="post" action="comm/commConfigCardorg.do">
			<!-- Head line -->
			<input type="hidden" name="verbId" value="<%=commConfigCardorg.getVerbId()%>">
			<input type="hidden" name="orderNo" value="<%=commConfigCardorg.getOrderNo()%>">
			<input type="hidden" name="asc" value="<%=commConfigCardorg.getAsc()%>">
			<input type="hidden" name="itemCodeHidden" value="<%=commConfigCardorg.getItemCodeHidden()%>">
			<!--查询条件-->
			<table width="95%" class="table2" align="center" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td id="list_L"></td>
					<td class="list_biaoti">
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commconfigcardorglist.text5"/>
					</td>
					<td id="list_C"></td>
					<td class="list_cc">
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.code"/>:
						<input name="itemCode" type="text" onkeypress="eventOnKeyPress('itemName')" value="<%=commConfigCardorg.getItemCode()%>">
					</td>
					<td class="list_cc">
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.name"/>:
						<input name="itemName" type="text" onkeypress="eventOnKeyPress('inputCode')" value="<%=commConfigCardorg.getItemName()%>">
					</td>
					<td class="list_cc">
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.inputCode"/>:
						<input name="inputCode" type="text" onkeypress="eventOnKeyPress('<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.submit"/>')" value="<%=commConfigCardorg.getInputCode()%>">
					</td>
					<td id="list_R"></td>
					<td class="button">
			...
			<table class="table" align="center" width="95%" cellspacing="1" cellpadding="0">
				...
				<tbody id="interval_row_id">
				   <c:if test="${pb.count<=0}">
					...
					<%
							if (commConfigCardorg.getItemCodeList() != null && commConfigCardorg.getItemCodeList().length > 0) {
							for (int i = 0; i < commConfigCardorg.getItemCodeList().length; i++) {
					%>
					<tr class="list_neirong">
						<td>
							<%=commConfigCardorg.getItemCodeList()[i]%>
						</td>
						<td>
							<%=commConfigCardorg.getItemNameList()[i]%>
						</td>
						<td>
							<%=commConfigCardorg.getSeqNoList()[i]%>
						</td>
						<td>
							<%=commConfigCardorg.getInputCodeList()[i]%>
						</td>
						<td>
							<img onClick="cmdView('<%=commConfigCardorg.getItemCodeList()[i]%>')" alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.detail"/>" src="include/images/cmdView_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
						<td>
							<img onClick="cmdEdit('<%=commConfigCardorg.getItemCodeList()[i]%>')" alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.change"/>" src="include/images/cmdEdit_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
						<td>
							<img onClick="cmdDel('<%=commConfigCardorg.getItemCodeList()[i]%>')" alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.delete"/>" src="include/images/cmdDel_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
					</tr>
					<%
						}
