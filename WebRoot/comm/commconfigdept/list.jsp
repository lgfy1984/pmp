...
<html>
	<head>
		...
		%>
		<title><bean:message  bundle="comm.commLocale" key="comm.jsp.commconfigdeptlist.text25"/></title>
...
function isMadeOf(val,str)
{
	...
	<body>
		...
		<form name="form" method="post" action="comm/commConfigDept.do">
			<!-- Head line -->
			<input type="hidden" name="verbId" value="<%=commConfigDept.getVerbId()%>">
			<input type="hidden" name="orderNo" value="<%=commConfigDept.getOrderNo()%>">
			<input type="hidden" name="asc" value="<%=commConfigDept.getAsc()%>">
			<input type="hidden" name="itemCodeHidden" value="<%=commConfigDept.getItemCodeHidden()%>">
			<!--查询条件-->
			<table width="95%" class="table2" align="center" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td id="list_L"></td>
					<td class="list_biaoti">
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commconfigdeptlist.xxxx1"/>
					</td>
					<td id="list_C"></td>
					<td class="list_cc">
			...
			<table width="95%" align="center" class="table" border="0" cellspacing="0" cellpadding="0">
				...
				<tr class="yuan_height">
					...
					<td width="35%" align="right" style="border-right: 1px solid #BFBFBF; padding: 0px 10px 0px 0px;">
						<a href="javascript:cmdAdd()"><img border=0 alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.add"/>" src="<bean:message key="security.jsp.securityconfigpublicclass.add" bundle="conf.security.security"/>" style="cursor: hand"> </a>
					</td>
				</tr>
			</table>
			<!--列表内容-->
				...
				<tbody id="interval_row_id">
				  <c:if test="${pb.count<=0}">
					...
					<tr class="list_neirong">
						<td>
							<%=commConfigDept.getItemCodeList()[i]%>
						</td>
						<td>
							<%=commConfigDept.getItemNameList()[i]%>
						</td>
						<td>
							<%=commConfigDept.getSeqNoList()[i]%>
						</td>
						<td>
							<%=commConfigDept.getInputCodeList()[i]%>
						</td>
						<td>
							<img onClick="cmdView('<%=commConfigDept.getItemCodeList()[i]%>')" alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.detail"/>" src="include/images/cmdView_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
						<td>
							<img onClick="cmdEdit('<%=commConfigDept.getItemCodeList()[i]%>')" alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.change"/>" src="include/images/cmdEdit_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
						<td>
							<img onClick="cmdDel('<%=commConfigDept.getItemCodeList()[i]%>')" alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.delete"/>" src="include/images/cmdDel_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
					</tr>
					<%
						}
