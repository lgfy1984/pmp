...
<jsp:useBean id="commConfigEconkind" scope="request" type="com.tianjian.comm.struts.form.CommConfigEconkindForm" />
...
<html>
	<head>
		...
		%>
		<title><bean:message  bundle="comm.commLocale" key="comm.jsp.commconfigeconkind.text13"/></title>
  ...
  <script type="text/javascript" src="${path}/js/default.js"></script>
	...
	<body>
		...
		<form name="form" method="post" action="comm/commConfigEconkind.do">
			<!-- Head line -->
			<input type="hidden" name="verbId" value="<%=commConfigEconkind.getVerbId()%>"/>
			<input type="hidden" name="orderNo" value="<%=commConfigEconkind.getOrderNo()%>"/>
			<input type="hidden" name="asc" value="<%=commConfigEconkind.getAsc()%>"/>
			...
			<!-- <table border="0" cellpadding="0" cellspacing="0" class="tblSearch" align="center">
				<tr>
					<td class="tblTitle" colspan="4">
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commconfigeconkindlist.text11"/>
					</td>
				</tr>
				<tr>	
					<td>
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.code"/>:
						<input name="itemCode" type="text" onkeypress="eventOnKeyPress('itemName')" value="<%=commConfigEconkind.getItemCode()%>"/>
				
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.name"/>:
						<input name="itemName" type="text" onkeypress="eventOnKeyPress('inputCode')" value="<%=commConfigEconkind.getItemName()%>"/>
					
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.inputCode"/>:
						<input name="inputCode" type="text" onkeypress="eventOnKeyPress('<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.submit"/>')" value="<%=commConfigEconkind.getInputCode()%>"/>
		...
		<div class='crm_search_div' style='overflow: hidden'>
			<div class="crm_input_item">
				<span>代码</span> <input class="crm_search_input_text"
					...
					value="<%=commConfigEconkind.getItemCode()%>" />
			</div>
			<div class="crm_input_item">
				<span>名称</span> <input class="crm_search_input_text"
					...
					value="<%=commConfigEconkind.getItemName()%>" />
			</div>
			<div class="crm_input_item">
				<span>输入码</span> <input class="crm_search_input_text"
					...
					value="<%=commConfigEconkind.getInputCode()%>" />
			</div>
			<div class="crm_input_item">
		...
		<div id="dvh1" class='crm_table_out'>
			<table style="table-layout: fixed;" class='crm_table_content'>
			    ...
			    <thead>
			    	<tr>
			            <th width="5%" height="26"><bean:message  bundle="comm.commLocale" key="comm.jsp.commom.code"/><div style="text-align:center;"><img style="vertical-align:center;" src="include/images/cmdOrderByAsc.gif" alt="Ascender" vspace="7" onclick="commandOrderBy('1', '0')" /><img style="vertical-align:center;" border="0" onclick="commandOrderBy('1', '1')" alt="Descender" src="include/images/cmdOrderByDesc.gif" /></div></th>
				...
				<tbody id="interval_row_id">
					...
					<%
							if (commConfigEconkind.getItemCodeList() != null && commConfigEconkind.getItemCodeList().length > 0) {
							for (int i = 0; i < commConfigEconkind.getItemCodeList().length; i++) {
								if(i%2==0){%>
									<tr class="odd">
								<%}else{ %>
									<tr class="even">
					...
					%>
						...
						<td>
							<%=commConfigEconkind.getItemCodeList()[i]%>
						</td>
						<td>
							<%=commConfigEconkind.getItemNameList()[i]%>
						</td>
						<td>
							<%=commConfigEconkind.getSeqNoList()[i]%>
						</td>
						<td>
							<%=commConfigEconkind.getInputCodeList()[i]%>
						</td>
						<!-- <td>
							<img onClick="cmdView('<%=commConfigEconkind.getItemCodeList()[i]%>')" alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.detail"/>" src="include/images/cmdView_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
						<td>
							<img onClick="cmdEdit('<%=commConfigEconkind.getItemCodeList()[i]%>')" alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.change"/>" src="include/images/cmdEdit_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
						<td>
							<img onClick="cmdDel('<%=commConfigEconkind.getItemCodeList()[i]%>')" alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.delete"/>" src="include/images/cmdDel_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td> -->
						<td style="cursor:pointer;"><input type="button" class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'" value="详细"  onclick="cmdView('<%=commConfigEconkind.getItemCodeList()[i]%>')"/></td>
						<td style="cursor:pointer;"><input type="button" class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'" value="修改"  onclick="cmdEdit('<%=commConfigEconkind.getItemCodeList()[i]%>')"/></td>
						<td style="cursor:pointer;"><input type="button" class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'" value="删除"  onclick="cmdDel('<%=commConfigEconkind.getItemCodeList()[i]%>')"/></td>
					</tr>
					<%
						}
						}
