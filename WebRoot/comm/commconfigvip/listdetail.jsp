...
function goPage2() {
  ...
  var _total=document.getElementById('_total'); 
    if (!isMadeOf(_tp.value,'1234567890')) {
      alert('<bean:message key="comm.jsp.commconfigsex.list.gopage2_1" bundle="conf.comm.CommMessageguoh"/>');
      return;
    }
    if (_tp.value<=0){
    	alert('<bean:message key="comm.jsp.commconfigsex.list.gopage2_2" bundle="conf.comm.CommMessageguoh"/>');
		return;
    }
    if(parseInt(_tp.value)>parseInt(_total.value)){
      alert('<bean:message key="comm.jsp.commconfigsex.list.gopage2_3" bundle="conf.comm.CommMessageguoh"/>');
      return;
    } 
	...
	<body>
		...
		<form name="form" method="post" action="comm/commConfigVip.do">
			...
			<table  border="0" cellpadding="0" cellspacing="0" class="tblSearch" align="center">
				<tr>
					<td class="tblTitle" colspan="4">
						<bean:message key="comm.jsp.commconfigsex.list.td1" bundle="conf.comm.CommMessageguoh"/>
					</td>
				</tr>
				<tr>
					<td>
						<bean:message key="comm.jsp.commconfigsex.list.td2" bundle="conf.comm.CommMessageguoh"/>
						<input name="itemCode" type="text" onkeypress="eventOnKeyPress('itemName')" value="<%=commConfigSex.getItemCode()%>">
					
						<bean:message key="comm.jsp.commconfigsex.list.td3" bundle="conf.comm.CommMessageguoh"/>
						<input name="itemName" type="text" onkeypress="eventOnKeyPress('inputCode')" value="<%=commConfigSex.getItemName()%>">
				
						<bean:message key="comm.jsp.commconfigsex.list.td4" bundle="conf.comm.CommMessageguoh"/>
						<input name="inputCode" type="text" onkeypress="eventOnKeyPress('btnSearch')" value="<%=commConfigSex.getInputCode()%>">
					
						<input type="button" class="btnSave" id="btnSearch" name="btnSearch"  name='<bean:message key="comm.jsp.commconfigsex.list.td5" bundle="conf.comm.CommMessageguoh"/>' value='<bean:message key="comm.jsp.commconfigsex.list.td6" bundle="conf.comm.CommMessageguoh"/>' onClick="submitQueryForm();" />
					</td>
				</tr>
			</table>
			<!--列表标题-->
			...
			<table border="0" cellpadding="0" cellspacing="0" class="tblSearchList" align="center">
			    ...
			    <thead>
			    	<tr class="lstName">
			            <th width="5%" height="26"><bean:message key="comm.jsp.commconfigsex.list.td9" bundle="conf.comm.CommMessageguoh"/><div><img src="include/images/cmdOrderByAsc.gif" alt="Ascender" vspace="7" onclick="commandOrderBy('1', '0')" /><img border="0" onclick="commandOrderBy('1', '1')" alt="Descender" src="include/images/cmdOrderByDesc.gif" /></div></th>
			            <th width="15%" height="26"><bean:message key="comm.jsp.commconfigsex.list.td10" bundle="conf.comm.CommMessageguoh"/><div><img src="include/images/cmdOrderByAsc.gif" alt="Ascender" vspace="7" onclick="commandOrderBy('2', '0')" /><img border="0" onclick="commandOrderBy('2', '1')" alt="Descender" src="include/images/cmdOrderByDesc.gif" /></div></th>
			            <th width="5%" height="26"><bean:message key="comm.jsp.commconfigsex.list.td11" bundle="conf.comm.CommMessageguoh"/><div><img src="include/images/cmdOrderByAsc.gif" alt="Ascender" vspace="7" onclick="commandOrderBy('3', '0')" /><img border="0" onclick="commandOrderBy('3', '1')" alt="Descender" src="include/images/cmdOrderByDesc.gif" /></div></th>
			            <th width="4%" height="26"><bean:message key="comm.jsp.commconfigsex.list.td12" bundle="conf.comm.CommMessageguoh"/><div><img src="include/images/cmdOrderByAsc.gif" alt="Ascender" vspace="7" onclick="commandOrderBy('4', '0')" /><img border="0" onclick="commandOrderBy('4', '1')" alt="Descender" src="include/images/cmdOrderByDesc.gif" /></div></th>
			            <th width="3%" height="26"><bean:message key="comm.jsp.commconfigsex.list.td13" bundle="conf.comm.CommMessageguoh"/></th>
			        </tr>
				</thead>
				<tbody id="interval_row_id">
					...
					<tr>
						<td style="text-align:left;padding-left:10px;">
							<%=commConfigSex.getItemCodeList()[i]%>
						</td>
						<td style="text-align:left;padding-left:10px;">
							<%=commConfigSex.getItemNameList()[i]%>
						</td>
						<td style="text-align:left;padding-left:10px;">
							<%=commConfigSex.getSeqNoList()[i]%>
						</td>
						<td style="text-align:left;padding-left:10px;">
							<%=commConfigSex.getInputCodeList()[i]%>
						</td>
						<td>
							<img onClick="cmdView('<%=commConfigSex.getItemCodeList()[i]%>')" alt='<bean:message key="comm.jsp.commconfigsex.list.img9" bundle="conf.comm.CommMessageguoh"/>' src="include/images/cmdView_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
					</tr>
					<%
						}
			...
			<table width="100%" align="center" class="tblScrollFooter">
				<tr>
					<td colspan="11" align="center" class="footer">
						...
						<input type="hidden" id="reHref" name="reHref" value="<%=request.getContextPath()%>/comm/commConfigSex.do?verbId=queryDetail" />
