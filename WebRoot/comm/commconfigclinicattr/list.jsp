...
<html>
	<head>
		...
		%>
		<title><bean:message  bundle="comm.commLocale" key="comm.jsp.commconfigclinicattrlist.xxxx"/></title>
...
function isMadeOf(val,str)
{
	...
	<body>
		<!-- Form中的action项的值必须设置-->
		<form name="form" method="post" action="comm/commConfigClinicattr.do">
			<!-- Head line -->
			<input type="hidden" name="verbId" value="<%=commConfigClinicattr.getVerbId()%>">
			<input type="hidden" name="orderNo" value="<%=commConfigClinicattr.getOrderNo()%>">
			<input type="hidden" name="asc" value="<%=commConfigClinicattr.getAsc()%>">
			<input type="hidden" name="itemCodeHidden" value="<%=commConfigClinicattr.getItemCodeHidden()%>">
			<!--查询条件-->
			<table width="95%" class="table2" align="center" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td id="list_L"></td>
					<td class="list_biaoti">
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commconfigclinicattrlist.text20"/>
					</td>
					<td id="list_C"></td>
					<td class="list_cc">
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.code"/>:
						<input name="itemCode" type="text" onkeypress="eventOnKeyPress('itemName')" value="<%=commConfigClinicattr.getItemCode()%>">
					</td>
					<td class="list_cc">
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.name"/>:
						<input name="itemName" type="text" onkeypress="eventOnKeyPress('inputCode')" value="<%=commConfigClinicattr.getItemName()%>">
					</td>
					<td class="list_cc">
						<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.inputCode"/>:
						<input name="inputCode" type="text" onkeypress="eventOnKeyPress('<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.submit"/>')" value="<%=commConfigClinicattr.getInputCode()%>">
					</td>
					<td id="list_R"></td>
					<td class="button">
				...
				<tbody id="interval_row_id">
				   <c:if test="${pb.count<=0}">
					...
					<tr class="list_neirong">
						<td>
							<%=commConfigClinicattr.getItemCodeList()[i]%>
						</td>
						<td>
							<%=commConfigClinicattr.getItemNameList()[i]%>
						</td>
						<td>
							<%=commConfigClinicattr.getSeqNoList()[i]%>
						</td>
						<td>
							<%=commConfigClinicattr.getInputCodeList()[i]%>
						</td>
						<td>
							<img onClick="cmdView('<%=commConfigClinicattr.getItemCodeList()[i]%>')" alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.detail"/>" src="include/images/cmdView_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
						<td>
							<img onClick="cmdEdit('<%=commConfigClinicattr.getItemCodeList()[i]%>')" alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.change"/>" src="include/images/cmdEdit_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
						<td>
							<img onClick="cmdDel('<%=commConfigClinicattr.getItemCodeList()[i]%>')" alt="<bean:message  bundle="comm.commLocale" key="comm.jsp.commom.delete"/>" src="include/images/cmdDel_s.jpg" border="0" style="cursor: hand; vertical-align: middle;" />
						</td>
					</tr>
					<%
						}
