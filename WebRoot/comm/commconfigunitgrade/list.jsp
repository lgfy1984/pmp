...
<jsp:useBean id="commConfigUnitgrade" scope="request" type="com.tianjian.comm.struts.form.CommConfigUnitgradeForm" />
...
<html>
	<head>
		...
		%>
		<title><bean:message key="comm.jsp.commconfigunitgrade.list.title" bundle="conf.comm.CommMessageguoh"/></title>
  ...
  <script type="text/javascript" src="${path}/js/default.js"></script>
	...
	<body>
		...
		<form name="form" method="post" action="comm/commConfigUnitgrade.do">
			<!-- Head line -->
			<input type="hidden" name="verbId" value="<%=commConfigUnitgrade.getVerbId()%>"/>
			<input type="hidden" name="orderNo" value="<%=commConfigUnitgrade.getOrderNo()%>"/>
			<input type="hidden" name="asc" value="<%=commConfigUnitgrade.getAsc()%>"/>
		...
		<div class='crm_search_div' style='overflow: hidden'>
		 <div class="crm_input_item">
		  	...
		  	<input class="crm_search_input_text" name="itemCode" type="text" onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
								onfocus="fEvent('focus',this)" onmouseout="fEvent('mouseout',this)" onkeypress="eventOnKeyPress('itemName')" value="<%=commConfigUnitgrade.getItemCode()%>" />
		 </div>
		 <div class="crm_input_item">
		  	<span>名称</span>
		  	<input class="crm_search_input_text" name="itemName" type="text" onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
								onfocus="fEvent('focus',this)" onmouseout="fEvent('mouseout',this)" onkeypress="eventOnKeyPress('inputCode')" value="<%=commConfigUnitgrade.getItemName() %>" />
		 </div>
		 <div class="crm_input_item">
		  	<span>输入码</span>
		  	<input class="crm_search_input_text" name="inputCode" type="text" onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
								onfocus="fEvent('focus',this)" onmouseout="fEvent('mouseout',this)" onkeypress="eventOnKeyPress('btnSave')" value="<%=commConfigUnitgrade.getInputCode() %>" />
		 </div>
		 <div class="crm_input_item">
	  ...
	  <div class='crm_table_out'>
		  ...
		  <tbody>
			...
			<%if (commConfigUnitgrade.getItemCodeList() != null && commConfigUnitgrade.getItemCodeList().length > 0) {
				for (int i = 0; i < commConfigUnitgrade.getItemCodeList().length; i++) {
					if(i%2==0){%>
						<tr class="odd">
					<%}else{ %>
						<tr class="even">
					<%}
					%>
					<td><%=commConfigUnitgrade.getItemCodeList()[i]%></td>
					<td><%=commConfigUnitgrade.getItemNameList()[i]%></td>
					<td><%=commConfigUnitgrade.getSeqNoList()[i]%></td>
					<td><%=commConfigUnitgrade.getInputCodeList()[i]%></td>
					<td style="cursor:pointer;"><input type="button" class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'" value="详细"  onclick="cmdView('<%=commConfigUnitgrade.getItemCodeList()[i] %>')"/></td>
					<td style="cursor:pointer;"><input type="button" class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'" value="修改"  onclick="cmdEdit('<%=commConfigUnitgrade.getItemCodeList()[i] %>')"/></td>
					<td style="cursor:pointer;"><input type="button" class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'" value="删除"  onclick="cmdDel('<%=commConfigUnitgrade.getItemCodeList()[i] %>')"/></td>
				  </tr>
				<%}
			}
			%>
