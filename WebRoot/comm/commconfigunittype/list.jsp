...
<jsp:useBean id="commConfigUnittype" scope="request" type="com.tianjian.comm.struts.form.CommConfigUnittypeForm" />
...
<html>
	<head>
		...
		%>
		<title><bean:message key="comm.jsp.commconfigunittype.list.title" bundle="conf.comm.CommMessageguoh"/></title>
...
function goPage2() {
  ...
  var _total=document.getElementById('_total'); 
    if (!isMadeOf(_tp.value,'1234567890')) {
      $.messager.alert('提示','<bean:message key="comm.jsp.commconfigrh.list.gopage2_1" bundle="conf.comm.CommMessageguoh"/>');
      return;
    }
    if (_tp.value<=0){
    	$.messager.alert('提示','<bean:message key="comm.jsp.commconfigrh.list.gopage2_1" bundle="conf.comm.CommMessageguoh"/>');
    ...
    if(parseInt(_tp.value)>parseInt(_total.value)){
      $.messager.alert('提示','<bean:message key="comm.jsp.commconfigrh.list.gopage2_3" bundle="conf.comm.CommMessageguoh"/>');
      return;
    } 
  ...
  <script type="text/javascript" src="${path}/js/default.js"></script>
	...
	<body>
		...
		<form name="form" method="post" action="comm/commConfigUnittype.do">
			<!-- Head line -->
			<input type="hidden" name="verbId" value="<%=commConfigUnittype.getVerbId()%>"/>
			<input type="hidden" name="orderNo" value="<%=commConfigUnittype.getOrderNo()%>"/>
			<input type="hidden" name="asc" value="<%=commConfigUnittype.getAsc()%>"/>
		...
		<div class='crm_search_div' style='overflow: hidden'>
		 <div class="crm_input_item">
		  	...
		  	<input class="crm_search_input_text" name="itemCode" type="text" onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
								onfocus="fEvent('focus',this)" onmouseout="fEvent('mouseout',this)" onkeypress="eventOnKeyPress('itemName')" value="<%=commConfigUnittype.getItemCode()%>" />
		 </div>
		 <div class="crm_input_item">
		  	<span>名称</span>
		  	<input class="crm_search_input_text" name="itemName" type="text" onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
								onfocus="fEvent('focus',this)" onmouseout="fEvent('mouseout',this)" onkeypress="eventOnKeyPress('inputCode')" value="<%=commConfigUnittype.getItemName() %>" />
		 </div>
		 <div class="crm_input_item">
		  	<span>输入码</span>
		  	<input class="crm_search_input_text" name="inputCode" type="text" onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
								onfocus="fEvent('focus',this)" onmouseout="fEvent('mouseout',this)" onkeypress="eventOnKeyPress('btnSave')" value="<%=commConfigUnittype.getInputCode() %>" />
		 </div>
		 <div class="crm_input_item">
	  ...
	  <div class='crm_table_out'>
		  ...
		  <tbody>
			...
			<%if (commConfigUnittype.getItemCodeList() != null && commConfigUnittype.getItemCodeList().length > 0) {
				for (int i = 0; i < commConfigUnittype.getItemCodeList().length; i++) {
					if(i%2==0){%>
						<tr class="odd">
					<%}else{ %>
						<tr class="even">
					<%}
					%>
					<td><%=commConfigUnittype.getItemCodeList()[i]%></td>
					<td><%=commConfigUnittype.getItemNameList()[i]%></td>
					<td><%=commConfigUnittype.getSeqNoList()[i]%></td>
					<td><%=commConfigUnittype.getInputCodeList()[i]%></td>
					<td style="cursor:pointer;"><input type="button" class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'" value="详细"  onclick="cmdView('<%=commConfigUnittype.getItemCodeList()[i] %>')"/></td>
					<td style="cursor:pointer;"><input type="button" class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'" value="修改"  onclick="cmdEdit('<%=commConfigUnittype.getItemCodeList()[i] %>')"/></td>
					<td style="cursor:pointer;"><input type="button" class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'" value="删除"  onclick="cmdDel('<%=commConfigUnittype.getItemCodeList()[i] %>')"/></td>
				  </tr>
				<%}
			}
			%>
