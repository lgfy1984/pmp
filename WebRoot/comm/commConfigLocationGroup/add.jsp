...
<jsp:useBean id="data" scope="request"
	class="com.tianjian.comm.struts.form.CommConfigLocationGroupForm" />
...
<html xmlns="http://www.w3.org/1999/xhtml">
	<head>
		...
		%>
		<title><bean:message key="comm.jsp.common.jmz"
				bundle="conf.comm.Comm" />add.jsp</title>
		<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
		<meta http-equiv="pragma" content="no-cache" />
		<meta http-equiv="cache-control" content="no-cache" />
		<meta http-equiv="expires" content="0" />
		<script language="javascript"
			src="<bean:message key="comm.js.comm.message" bundle="conf.comm.Comm"/>"></script>
		<script language="javascript"
...
function saveForm(){
		...
		if($('#commClvId').combobox('getValue')== ""){
		 	$.messager.alert('提示',"<bean:message key="comm.jsp.common.selectVillage" bundle="conf.comm.Comm"/>");
		 	return ;
		}
		if(document.form.seqNo.value == ""){
		 	$.messager.alert('提示',"<bean:message key="comm.jsp.common.selectSeqNo" bundle="conf.comm.Comm"/>");
		 	return ;
		}
		...
		if(document.form.itemCode.value == ""){
		 	$.messager.alert('提示',"<bean:message key="comm.jsp.common.alert12" bundle="conf.comm.Comm"/>");
		 	return ;
		}
		if(isNaN(document.form.itemCode.value)){
	       $.messager.alert('提示',"<bean:message key="comm.jsp.common.alert9" bundle="conf.comm.Comm"/>"); 
	       return true;
	    }
	    if(document.form.itemCode.value.length !=12){
		 	$.messager.alert('提示',"<bean:message key="comm.jsp.common.alert8" bundle="conf.comm.Comm"/>");
		 	return ;
		}
	    if(document.form.itemName.value == ""){
		 	$.messager.alert('提示',"<bean:message key="comm.jsp.common.selectPersonName" bundle="conf.comm.Comm"/>");
		 	return ;
		}
	...
	function setItemCode(){
			var country=document.getElementById("commClvId").value;
			var xmlHttp = newXMLHttpRequest();
			var url = "<%=request.getContextPath()%>/comm/commConfigLocationGroup.do?verbId=setItemCode&country="+country;
      ...
      <div class='crm_edit_panel'>
        ...
        onSelect: function(rec){    
            ...
            $('#commCityId').combobox('reload', url);    
							...
							<%if (data.getCommProvinceIds() != null
					&& data.getCommProvinceIds().length > 0) {
        ...
        onSelect: function(rec){    
            ...
            $('#commCltId').combobox('reload', url);    
							...
							<%if (data.getCommCountyIds() != null
					&& data.getCommCountyIds().length > 0) {
				for (int i = 0; i < data.getCommCountyIds().length; i++) {
					...
					String tempName = data.getCommCountyNames()[i];%>
							<option value="<%=tempId%>"
								<%=tempId.equals(data.getCommCountyId()) ? "selected"
									: ""%>>
								<%=tempName%>
							</option>
							<%}
      	  		...
      	  		<td class='crm_edit_item_content'>
						<select class="easyui-combobox" editable="false" style="width: 206px;height:30px;" name="commCltId"
							id="commCltId"
							onkeypress="eventOnKeyPress('seqNo')" data-options="valueField:'id',textField:'itemName',url: 'get_data4.php',    
        onSelect: function(rec){ 
            var url = 'comm/commConfigLocationGroup.do?verbId=setVillage&town='+rec.id; 
            $('#commClvId').combobox('clear');   
            $('#commClvId').combobox('reload', url);    
        }">
      	  	...
      	  	<tr>
      	  		<td class='crm_edit_item_name'><label class="redlable">*</label>所属村</td>
      	  		<td class='crm_edit_item_content'>
						<select class="easyui-combobox" editable="false" style="width: 206px;height:30px;" name="commClvId"
							id="commClvId"
				...
				for (int i = 0; i < data.getCommClvIds().length; i++) {
					...
					String tempName = data.getCommClvNames()[i];%>
							<option value="<%=tempId%>"
								<%=tempId.equals(data.getCommClvId()) ? "selected"
							: ""%>>
								<%=tempName%>
							</option>
							<%}
      	  	...
      	  	<tr>
      	  		<td class='crm_edit_item_name'><label class="redlable">*</label>居民组代码</td>
      	  		<td class='crm_edit_item_content'>
      	  	...
      	  	<tr>
      	  		...
      	  		<td class='crm_edit_item_content' colspan="3">
      	  			<input type="text" class="text" name="comments" id="comments"   onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
							...
							onkeypress="eventOnKseyPress('btnSaveForm')"/>
				</td> 
      	  	</tr>  	  	
      	  </table>
      </div>
			...
			<table border="0" cellspacing="1" cellpadding="0" class="table" align="center">
				<tr>
					<td height="30px" class="biaoti" colspan="4">
						<font color="red">※</font>&nbsp;&nbsp;<bean:message key="comm.jsp.common.jmz" bundle="conf.comm.Comm"/>&nbsp;&nbsp;
						<font color="red">※</font>
					</td>
				</tr>				
				<tr>
