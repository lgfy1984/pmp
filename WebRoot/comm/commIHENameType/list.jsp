...
<jsp:useBean id="data" scope="request" type="com.tianjian.comm.struts.form.CommIHENameTypeForm" />
...
<html>
	<head>
		...
		%>
		<title><bean:message  bundle="comm.comIHE" key="comm.jsp.commIHENameType.list"/></title>
			...
			<table width="95%" class="table2" align="center" border="0" cellspacing="0" cellpadding="0">
				<tr>
					...
					<td class="list_biaoti">
						<bean:message  bundle="comm.comIHE" key="comm.jsp.commIHENameType.list"/>
					</td>
					<td id="list_C"></td>
					<td class="list_cc">
						<bean:message  bundle="comm.comIHE" key="comm.jsp.commIHENameType.segment2"/>:
						<input name="nameTypeCode" type="text" onkeypress="eventOnKeyPress('nameTypeName')" value="<%=data.getNameTypeCode()%>" style="height:17px;">
					</td>
					<td class="list_cc">
						<bean:message  bundle="comm.comIHE" key="comm.jsp.commIHENameType.segment3"/>:
						<input name="nameTypeName" type="text" onkeypress="eventOnKeyPress('inputCode')" value="<%=data.getNameTypeName()%>" style="height:17px;">
					</td>
					<td class="list_cc">
						<bean:message  bundle="comm.comIHE" key="comm.jsp.commIHENameType.segment4"/>:
						<input name="inputCode" type="text" value="<%=data.getInputCode()%>" onkeypress="eventOnKeyPress('btnSearch')" style="height:17px;">
					</td>
					<td id="list_R"></td>
					<td class="button">
			...
			<table class="table" align="center" width="95%" cellspacing="1" cellpadding="0">
				   ...
				   <c:if test="${pb.count<=0}">
					...
					<tr class="list_neirong">
						<td class="td_shenglue leftPadding" height="25">
							<%=data.getNameTypeCodeList()[i]%>
						</td>
						<td class="td_shenglue leftPadding" >
							<%=data.getNameTypeNameList()[i]%>
						</td>
						<td class="td_shenglue leftPadding" >
							<%=data.getSeqNoList()[i]%>
						</td>
				...
				<tr <c:if test="${pb.count<=0}">style="display:none"</c:if>>
					<td colspan="8" align="center" bgcolor="#ffffff" height="35px">
						...
						%>
						<input type="hidden" id="pageId" name="pageId" value="page_2828810b3696042f013696042f540000" />
						<input type="hidden" id="reHref" name="reHref" value="<%=request.getContextPath()%>/comm/commIHENameTypeAction.do?verbId=query" />
