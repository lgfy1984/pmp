...
<html>
	...
	<body onload="showHspMessage('${data.message}')">
		<form name="form" method="post" action="commConfigStafftype.do">
				...
				<div class='crm_search_div' style='overflow: hidden'>
					<div class="crm_input_item">
						...
						<input type="text" id="itemCode" name="itemCode"
							...
							class="crm_search_input_text" value="${commConfigStafftype.itemCode}" />
					</div>
					<div class="crm_input_item">
						<span>名称</span>
						<input type="text" id="itemName" name="itemName"
							onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
							onfocus="fEvent('focus',this)" onmouseout="fEvent('mouseout',this)"
							class="crm_search_input_text" value="${commConfigStafftype.itemName}" />
					</div>
					<div class="crm_input_item">
						<span>输入码</span>
						<input type="text" id="inputCode" name="inputCode"
							onblur="fEvent('blur',this)" onmouseover="fEvent('mouseover',this)" 
							onfocus="fEvent('focus',this)" onmouseout="fEvent('mouseout',this)"
							class="crm_search_input_text" value="${commConfigStafftype.inputCode}" />
					</div>
					<div class="crm_input_item">
				...
				<div class='crm_table_out'>
					<table class='crm_table_content'>
						...
						<tr>
							...
							<c:forEach items="${commConfigStafftype.ccsList}" var="list" begin="0" step="1"
								varStatus="status">
								...
								<td id="itemCode${status.index}">
									${list.itemCode}
								</td>

								<td id="itemName${status.index}">
									<label title="${list.itemName}">
										<c:choose>
											<c:when test="${fn:length(list.itemName) > 10}">
												<c:out
													value="${fn:substring(list.itemName, 0, 10)}......" />
											</c:when>
											<c:otherwise>
								...
								<td id="seqNo${status.index}">
									${list.seqNo}
								</td>
								<td id="inputCode${status.index}">
									${list.inputCode}
								</td>
								...
								<td id="deletes${status.index}">
									<input type="button"  class="button_grey2_s0" onmouseout="this.className='button_grey2_s0'" onmousedown="this.className='button_grey2_s1'"
										onclick="deleteRow('${list.itemCode}')" value="删除">
	...
	function toAddpage(){
		window.location.href="${path}/comm/commConfigStafftype.do"+"?verbId="+"addInit";
	}
</script>
