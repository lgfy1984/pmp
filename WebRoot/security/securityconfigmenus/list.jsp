				   ...
				   <c:if test="${pb.count<=0}">
					...
					<logic:notEmpty name="dataForm" property="dataList">
						...
						<tr class="list_neirong">
							...
							<td>
								<img onClick="cmdEdit('${data.id}')"
									 alt='<bean:message key="jsp.update" bundle="conf.Init"/>'  src="include/images/cmdEdit_s.jpg" border="0"
									style="cursor: hand; vertical-align: middle;" />
							</td>
							<td>
								<img onClick="confirmdel('${data.id}')"
									 alt='<bean:message key="jsp.delete" bundle="conf.Init"/>'  src="include/images/cmdDel_s.jpg" border="0"
									style="cursor: hand; vertical-align: middle;" />
							</td>
						</tr>
					</logic:iterate>
			...
			<table width="100%" align="center" class="tblScrollFooter" <c:if test="${pb.count<=0}">style="display:none"</c:if>>
				<tr>
					...
					%>
						...
						<input type="hidden" id="reHref" name="reHref" value="<%=request.getContextPath()%>/security/securityConfigMenus.do?verbId=query" />
